...
- The runner program-output contract (`diagnostics.json` / `perf.json` / `raw/` composition + the JSON serialization rules) is canonical in `docs/workflow/RUNNER_OUTPUT_CONTRACT.md`; a language's JSON writer rules are its binding, `docs/backends/language/<language>/RUNNER_OUTPUT.md`. It reaches `Validate.judge` as an INLINED slice under the pure judge (§1 and §3 — what `diagnostics.json` must carry and what `raw/` must hold; issue #169) and it reaches the pure `Generate` leaves of an `infrastructure` harness self-test — the only node kind that authors its own runner — INLINED WHOLE, followed by the target language's runner-output binding (issue #289), as `runner_output_contract_document` of the `harness` bundle shape's producer and reviewer templates (issue #169). There is no force-read set on any path: the residual agentic judge and the residual agentic runner-authoring `Generate` leaf it was kept for are deleted with the transport (Z4, issue #171). **M3d node-aware:** it is **not** injected for an *M3c physics* `Generate` leaf — that leaf authors only the model and the checks module (its runner is host-rendered glue over the certified harness), so its runner-side contract is `CHECKS_MODULE_CONTRACT.md`, not this doc (see `leaf_contract_doc_refs`). It supersedes the runner-output prose previously duplicated in `phase_02_generate.md` / `phase_04_validate.md` / `PERFORMANCE_DIAGNOSTICS.md`; new runner-output rules go there, and those docs reference it.
- The checks-module contract `docs/workflow/CHECKS_MODULE_CONTRACT.md` reaches its leaves INLINED, each half to the leaf it binds (below). It was force-read by every `Generate` leaf of the agentic transport, not only an M3c one; that transport is deleted (Z4, issue #171), and inlining is the only route left. Its §1-4 are the language-neutral fixed checks-module ABI an *M3c physics* leaf authors its checks module against, and its §5 points to the target language's binding, `docs/backends/language/<language>/CHECKS_ABI.md` (issue #289): the binding's §1-4 spell §1-4 in that language, and its §5 (legality and gate guards — the idioms the deterministic `Generate.gate` lint and syntax checks enforce, such as the binding of an intentionally-unused dummy argument) binds **every leaf-authored source of that language, of any node**, including the hand-authored runner of an `infrastructure` self-test. `Validate.judge` never sees the checks source, so it is not injected there. Force-reading was the **agentic** leaf's route and no longer exists (Z4, issue #171); each half of the document reaches its leaf INLINED: a `pure` M3c producer is told the five ABI names, the bound-state convention, and shown the host-rendered runner; since `pure-24` a `pure` `Generate.verify` reviewer on the `m3c` shape receives §1-4 — since `pure-50` followed by the binding's §1-4 — as `checks_module_contract_document` (issue #142; the `compile.generate` producer receives the neutral §1-4 only, Compile being target-free) — the `harness` shape's reviewer does not, having no checks module to review, and receives the runner-output contract in its place; and since `pure-37` a `pure` producer on the `harness` shape — the runner-authoring leaf §5 names — receives the binding's §5 as `gate_guards_document`, because a `pure` leaf force-reads nothing and would otherwise be held to that rule set with no document stating it (issue #169). Since `pure-25` the same reviewer also receives `docs/workflow/phases/phase_02_generate.md` §2-2's severity rubric inlined as `severity_rubric_document`, which is the sole rule for the `issue_severity` its verdict carries (issue #143).
- **Where a leaf-actionable rule goes now that nothing is force-read** (the minimization this supersedes: `docs/design/leaf_must_read_restructure.md`). A rule reaches a leaf only by being in its launch template, or in a document the host INLINES into that template — `Conductor._build_pure_*_context` is the complete list of what is inlined, and `docs/workflow/LAUNCH_PROMPT_REFERENCE.md` describes the shapes. So: put a one-substep rule in that substep's `pure_*.txt` template; a rule binding both compile leaves in `phase_01_compile.md` (inlined whole); a runner-output rule in `RUNNER_OUTPUT_CONTRACT.md` (inlined whole for the `harness` shape, sliced for the judge); a checks-ABI rule in `CHECKS_MODULE_CONTRACT.md` §1-4 when it holds for every language, and in the language's binding `docs/backends/language/<language>/CHECKS_ABI.md` §1-4 when it is that language's spelling; a legality / gate idiom binding every leaf-authored source of a language in that binding's §5 (issue #289 — the neutral contract's §5 is a pointer no leaf receives); a generate authoring or review rule of a language in its prompt fragments, `tools/prompt_templates/backends/language/<language>/`; a runner JSON writer rule in `docs/backends/language/<language>/RUNNER_OUTPUT.md`. A rule written anywhere else reaches no leaf at all, silently — which is the failure mode this bullet exists to prevent, unchanged in kind from when the list was a must-read set.
...
## Auxiliary Skills
- `Workflow audit`: `skills/workflow-audit/SKILL.md` (backend-neutral; runs `tools/audit_orchestration.py` over the in-repo records of one orchestration)
...
- `Workflow timing & token audit`: `skills/workflow-timing-audit/SKILL.md` (Claude Code-only post-run diagnostic; breaks an orchestration down into per-leaf elapsed time and output tokens via `phase_state_log.jsonl` + the per-leaf session transcripts, collapsing the transcript multiple-counting traps; an HTTP leaf has no transcript, so its per-request tokens, elapsed and throughput come from the run's `launches/` artifacts instead; across runs, `scripts/attempt_cost_report.py` splits leaf cost into first launches and retries per `step.substep` over a chosen node set, the granularity the SKILL's §"Comparing cost across runs" requires for a cost comparison (it reads provider-neutral `agent_runs.jsonl` rows; only its warm-resume correction is specific to the claude CLI envelope); read-only, does not modify any artifact)
