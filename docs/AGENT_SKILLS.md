...
- The common contract that every child `step agent` / `substep agent` must apply (capability_token usage, read/write permission guards, the direct `Write` / `Edit` artifact-write procedure, tmp-area rules, the no-inline-write / no-cross-agent-read constraints, dev-mode fail rules, **the leaf-actionable workflow invariants and the `<stage>_meta.json` key rules**) uses `docs/AGENT_CONTRACT.md` as the **single common leaf must-read**. It is child-readable (under `docs/`, in every child's `read_manifest`) and is referenced by the rendered launch prompt rather than inlined per child; the security-critical constraint lines that `record-launch`'s `_required_launch_prompt_constraint_lines` guard requires are additionally kept inline in the launch prompt (`tools/prompt_templates/`).
- The runner program-output contract (`diagnostics.json` / `perf.json` / `raw/` composition + the Fortran JSON-descriptor rules) is canonical in `docs/workflow/RUNNER_OUTPUT_CONTRACT.md`. It reaches `Validate.judge` as an INLINED slice under the pure judge (§1 and §3 — what `diagnostics.json` must carry and what `raw/` must hold; issue #169) and is the force-read must-read for a residual agentic judge and for a **runner-authoring** `Generate` leaf (the `infrastructure` harness self-test — the only node kind that authors its own runner). **M3d node-aware:** it is **not** injected for an *M3c physics* `Generate` leaf — that leaf authors only `<spec_id>_model.f90` + `<spec_id>_checks.f90` (its runner is host-rendered glue over the certified harness), so its runner-side contract is `CHECKS_MODULE_CONTRACT.md`, not this doc (see `leaf_contract_doc_refs`). It supersedes the runner-output prose previously duplicated in `phase_02_generate.md` / `phase_04_validate.md` / `PERFORMANCE_DIAGNOSTICS.md`; new runner-output rules go there, and those docs reference it.
- The checks-module contract `docs/workflow/CHECKS_MODULE_CONTRACT.md` is force-read by **every** `Generate` leaf (`generate` and `verify`), not only an M3c one. Its §1-4 are the fixed checks-module ABI an *M3c physics* leaf authors `<spec_id>_checks.f90` against; its §5 (Fortran legality and gate guards — the idioms the deterministic `Generate.gate` lint and syntax checks enforce, such as the `associate` binding of an intentionally-unused dummy argument) binds **every leaf-authored Fortran source of any node**, including the hand-authored runner of a non-M3c leaf. `Validate.judge` never sees the checks source, so it is not injected there — and since Z3 the judge is a `pure` leaf with no force-read set at all, so this is a statement about the residual agentic path. Force-reading is the **agentic** leaf's route: a `pure` producer is told the ten ABI names and shown the host-rendered runner instead, and since `pure-24` a `pure` `Generate.verify` reviewer receives §1-4 inlined as `checks_module_contract_document` (issue #142). Since `pure-25` the same reviewer also receives `docs/workflow/phases/phase_02_generate.md` §2-2's severity rubric inlined as `severity_rubric_document`, which is the sole rule for the `issue_severity` its verdict carries (issue #143).
- **Leaf must-read minimization (canonical: `docs/design/leaf_must_read_restructure.md`).** `WORKFLOW_CORE.md`, `ORCHESTRATION.md`, and the per-phase `phase_*.md` are **not** force-read by LLM leaves (they are orchestration/operator canonical and stay readable under `docs/`); the leaf-actionable subset of `WORKFLOW_CORE.md` lives in `AGENT_CONTRACT.md`. The phase doc is force-read only for `Compile` (`phase_01`'s IR schema). When adding a leaf-actionable rule, prefer `AGENT_CONTRACT.md` (every leaf), the per-substep `SKILL.md` (one substep), `RUNNER_OUTPUT_CONTRACT.md` (runner output), or `CHECKS_MODULE_CONTRACT.md` (a rule binding every `Generate` leaf: the checks ABI in §1-4, the Fortran legality / gate idioms in §5) so it reaches the leaf without re-bloating the must-read set.
...
- On the `codex` backend, use `leaf_config/codex/hooks.json` as the canonical source for the LEAF hook invocation definitions, and require `feature_states.hooks=true` in the `preflight` decision.
...
- `Compile generate`: `skills/workflow-compile-generate/SKILL.md` (residual **agentic** compile leaf only — an entry whose `capabilities:` drops `pure`; on a `pure-function leaf` the leaf reads no SKILL — the host inlines the closed context into the launch prompt, see `docs/workflow/LAUNCH_PROMPT_REFERENCE.md`)
...
- `Compile verify`: `skills/workflow-compile-verify/SKILL.md` (residual **agentic** compile leaf only — an entry whose `capabilities:` drops `pure`; on a `pure-function leaf` the leaf reads no SKILL, as with `Compile generate` above)
- `Generate generate`: `skills/workflow-generate-generate/SKILL.md` (residual **agentic** generate leaf only — a non-`M3c` node; on a `pure-function leaf` the leaf reads no SKILL — the host inlines the closed context into the launch prompt, see `docs/workflow/LAUNCH_PROMPT_REFERENCE.md`)
- `Generate gate`: conductor in-process (no SKILL; deterministic lint check `run_linter`, syntax check `run_syntax_check` compiler front-end gate gfortran `-fsyntax-only`, and static check `validate_workspace_root` + `validate_pipeline_semantics --stage post_generate` — the static check runs only when the lint and syntax checks both pass; see `docs/workflow/phases/phase_02_generate.md`)
- `Generate verify`: `skills/workflow-generate-verify/SKILL.md` (residual **agentic** generate leaf only — a non-`M3c` node; on a `pure-function leaf` the leaf reads no SKILL, as with `Generate generate` above)
...
- `Validate judge`: `skills/workflow-validate-judge/SKILL.md` — read only by a residual AGENTIC judge. The pure judge (Z3, issue #169) reads no `SKILL`; its contract is `tools/prompt_templates/pure_validate_judge.txt`, the same arrangement the `Compile` and `Generate` rows describe.
