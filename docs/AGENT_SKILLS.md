...
- The runner program-output contract (`diagnostics.json` / `perf.json` / `raw/` composition + the Fortran JSON-descriptor rules) is canonical in `docs/workflow/RUNNER_OUTPUT_CONTRACT.md`. It is the leaf must-read for `Validate.judge` and for a **runner-authoring** `Generate` leaf (the `infrastructure` harness self-test — the only node kind that authors its own runner). **M3d node-aware:** it is **not** injected for an *M3c physics* `Generate` leaf — that leaf authors only `<spec_id>_model.f90` + `<spec_id>_checks.f90` (its runner is host-rendered glue over the certified harness), so its runner-side contract is `CHECKS_MODULE_CONTRACT.md`, not this doc (see `leaf_contract_doc_refs`). It supersedes the runner-output prose previously duplicated in `phase_02_generate.md` / `phase_04_validate.md` / `PERFORMANCE_DIAGNOSTICS.md`; new runner-output rules go there, and those docs reference it.
- The checks-module contract `docs/workflow/CHECKS_MODULE_CONTRACT.md` is force-read by **every** `Generate` leaf (`generate` and `verify`), not only an M3c one. Its §1-4 are the fixed checks-module ABI an *M3c physics* leaf authors `<spec_id>_checks.f90` against; its §5 (Fortran legality and gate guards — the idioms the deterministic `Generate.gate` lint and syntax checks enforce, such as the `associate` binding of an intentionally-unused dummy argument) binds **every leaf-authored Fortran source of any node**, including the hand-authored runner of a non-M3c leaf. `Validate.judge` never sees the checks source, so it is not injected there. Force-reading is the **agentic** leaf's route: a `pure` producer is told the ten ABI names and shown the host-rendered runner instead, and since `pure-24` a `pure` `Generate.verify` reviewer receives §1-4 inlined as `checks_module_contract_document` (issue #142). Since `pure-25` the same reviewer also receives `docs/workflow/phases/phase_02_generate.md` §2-2's severity rubric inlined as `severity_rubric_document`, which is the sole rule for the `issue_severity` its verdict carries (issue #143).
...
- `Workflow escalate (failure diagnostician)`: `skills/workflow-escalate/SKILL.md` (the read-only, one-shot escalate LLM the conductor consults when the deterministic decision tables cannot classify a phase failure — it emits a routing directive deciding rollback distance, severity `minor|major|critical`, and reuse-vs-discard of existing artifacts). **Conductor-consumed, not a phase leaf**: the conductor renders this SKILL body host-side into the diagnostician prompt (no `skill_ref` launch wiring, so it is intentionally absent from the phase→SKILL launch table).
