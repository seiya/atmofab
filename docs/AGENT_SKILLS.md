...
- `Compile static`: conductor in-process (no SKILL; deterministic `validate_workspace_root` + `validate_pipeline_semantics --stage compile`, see `docs/workflow/phases/phase_01_compile.md`)
...
- `Generate generate`: `skills/workflow-generate-generate/SKILL.md` (residual **agentic** generate leaf only — a node with no `CodegenBundle` shape, of which the catalog holds none since issue #169, so this SKILL has no in-tree consumer; on a `pure-function leaf` the leaf reads no SKILL — the host inlines the closed context into the launch prompt, see `docs/workflow/LAUNCH_PROMPT_REFERENCE.md`)
...
- `Generate verify`: `skills/workflow-generate-verify/SKILL.md` (residual **agentic** generate leaf only — a node with no `CodegenBundle` shape, of which the catalog holds none since issue #169, so this SKILL has no in-tree consumer; on a `pure-function leaf` the leaf reads no SKILL, as with `Generate generate` above)
