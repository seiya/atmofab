...
- The checks-module contract `docs/workflow/CHECKS_MODULE_CONTRACT.md` is force-read by **every** `Generate` leaf (`generate` and `verify`), not only an M3c one. Its §1-4 are the fixed checks-module ABI an *M3c physics* leaf authors `<spec_id>_checks.f90` against; its §5 (Fortran legality and gate guards — the idioms the deterministic `Generate.gate` lint and syntax checks enforce, such as the `associate` binding of an intentionally-unused dummy argument) binds **every leaf-authored Fortran source of any node**, including the hand-authored runner of a non-M3c leaf. `Validate.judge` never sees the checks source, so it is not injected there.
