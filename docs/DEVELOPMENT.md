...
- How a change to this repository is reviewed — the `metforge-review-loop` skill under `.claude/skills/`.
- What a change to the enforcement machinery must satisfy — the `metforge-enforcement-change` skill under `.claude/skills/`.
...
| `~/.met-forge/` | runtime state | the host, per orchestration | out of tree |
...
- **Measure before asserting.** Do not write a number, a count, or a causal claim into a document, a commit message, or `TODO.md` that was not produced by running something. The verification commands are in the `metforge-review-loop` and `metforge-enforcement-change` skills.
