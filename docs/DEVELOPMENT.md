...
**Step 6 is a DEVELOPER requirement and it disagrees with step 1 on purpose.** `docs/RUNBOOK.md` §0-1 is written for someone running a workflow, and it says in as many words that an operator installs the linter of the target they run — `fortitude` for the Fortran profile, the CUDA compiler driver for the CUDA C++ one — and neither of the other two — correct, because the linter a run selects follows from its `toolchain.language` and no target profile in this tree names a language those two lint. (The CUDA compiler driver is left out of the suite's requirement below: its rows skip, with a declared reason, where it is not installed, since CI carries no CUDA toolkit.) The SUITE is a different question: each linter backend's tests drive the real tool and deliberately FAIL rather than skip when it is absent, because a machine without the tool cannot certify anything and a green suite there would report that a gate is fine when nothing ran it (`.claude/skills/atmofab-enforcement-change` judgment rule 2). So a fresh clone that installs only what §0-1 lists gets a red suite whose message points back at §0-1. Install all three — two of them from the dev requirements file:
...
- **CI runs the suite, and it does not run everything.** `.github/workflows/tests.yml` runs `python3 -m pytest tools/tests/ -q -rs` on every pull request and on a push to `main`, with a GUARD step afterwards that compares what the suite executed against what a pristine configuration collects and fails if the report is missing, short, or carries failures — so a suite that was skipped, deselected or had its failure swallowed cannot leave a green check, on `ubuntu-22.04` whose SYSTEM interpreter is Python 3.10 — the version every measurement in this repository was taken on — with the packages `requirements.txt` and `requirements-dev.txt` pin. It is not a second opinion about other machines: its job is to make the operator's own claim on a round the operator forgot to measure, which is why there is no version matrix. **A measured figure is still recorded with the commit it was taken at**, and that has not changed: CI reports the suite, not the numbers this repository quotes.
  - **What CI does NOT cover**, so a green check is not read as more than it is: a billed `LLM` run (no leaf is launched); the mutation check, which is a procedure an author runs over their own diff, **and the tests of the two review instruments beside their scripts under `.claude/skills/` (`README.md` §Tests)**; the linter-statistics delta — and in fact this repository's own linter does not run in CI at all, so the 1497 findings that figure counts are guarded by nobody but the author; a host whose `static lint` tool is outside its backend's declared range, since CI installs one inside it; the CUDA compiler driver, which CI does not install (the CUDA toolkit is not part of the runner image), so the rows that run it are skipped there with the declared reason `the CUDA compiler driver is not installed` and are measured on a host that has it (issue #289, R4-b PR-4); and the differential harness that checks the language front end against an independent one, which needs a program neither the runner nor the development host has ([issue #181](https://github.com/seiya/atmofab/issues/181#issuecomment-5559629951) records the four front ends that were measured and why `flang` was kept out). The sandbox IS covered — the runner lifts `kernel.apparmor_restrict_unprivileged_userns` so `bwrap` runs rather than skipping. Two more, added after a review round asked whether the list was complete: **a branch pushed with no open pull request is not checked at all** (the triggers are `pull_request` and a push to `main`), and **a `pull_request` run certifies the MERGE with `main` as it stood when the run started** — nothing re-runs it when `main` moves, so with no protection rule a green check can describe a tree that never becomes `main`, and **no real leaf-`LLM` CLI is exercised anywhere** — the tests that used to invoke `claude` or `codex` did so only because the developer's machine had it, and each now supplies what it needs, so CI proves nothing about those CLIs' actual behaviour. (An earlier version of this clause said "no real backend CLI", which is FALSE in this repository's own sense of `backend`: the `static lint` tools and the compiler this job installs ARE `backend` ids, the apt line above installs them so they run, and each linter backend's tests drive the real binary. A reader would have concluded the apt line was dead weight.)
  - **And a RED check is not read as less than it is, either**: nothing blocks merging a pull request whose run failed, and nothing blocks a direct push to `main` — there is no protection rule and no ruleset (measured 2026-09-06). The bullet above records that about `main`; it belongs here too, because the strongest thing this CI does not do is stop anything.
...
## Operations Rules
- **One driver per workspace.** Never drive an orchestration by hand while a conductor is running it, and never start a second orchestration against the same workspace. The exclusive start claim (`~/.atmofab/start_claims/`, a flock the OS releases when the process dies) refuses a second driver for the same spec; what it cannot refuse is a hand-driven edit, and two writers in `workspace/` corrupt each other's artifacts — an id another run is producing under, a stage meta certified from a tree a second writer changed. While a run is in flight, treat the workspace as read-only. (The reason stated here until [issue #171](https://github.com/seiya/atmofab/issues/171) PR-2 was write ATTRIBUTION: a workspace-global baseline diff that concurrent runs would make mis-attribute each other's writes. That diff is deleted — no leaf holds write authority for it to audit — and the rule stands on the artifact collision instead.)
...
- **A version identifies content, not a re-run.** Do not bump a version to force re-certification while the content is unchanged; whether a certified output still stands is the derivation key's question (`docs/ORCHESTRATION.md` §13a), and `--rederive` forces a phase without touching any version. A change of REPRESENTATION is a content change and does bump.
