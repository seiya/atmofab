...
4. **Do not set `METFORGE_ORCHESTRATION_ASSUME_BWRAP`.** That env var is a test-only
...
5. For Codex, set `CODEX_HOME` to the writable Codex state directory when a non-default
   location is required. `METFORGE_HOME` is a deprecated compatibility alias. The two variables
...
| Real auth + `--session-id` transcript worked | `<projects-root>/<slug>/<session_id>.jsonl` exists (for a workflow leaf `<projects-root>` is `orchestration_meta.json#claude_workflow_home` + `/projects`, issue #63; for an operator's own session `~/.claude/projects`) and has assistant turns for each leaf (auth/config-home bind is functional). **Operator context only**: that path is the backend CLI's credential/session home, which the Bash read guard rejects fail-closed whenever `METFORGE_WORKFLOW_MODE=1` (policy `forbid_backend_credential_direct_read`; canonical: `docs/HOOKS.md` §"Layer boundary"). Check it from an operator terminal outside a workflow run |
