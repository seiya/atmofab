...
Status: **proposed** (reference architecture). Recorded 2026-07-12 and amended 2026-07-13 from a first-principles redesign study against the stated final goal. Parts of this architecture are already realized by the current implementation and are identified in place (R2 verdicts, R3-core test kinds, R5 exemplar injection, R1/M3c harness structure with the A6 amendment); of the Z-series migration items, Z0 and Z2 are implemented (identified in place below); **Z1's implementation exists but is NOT yet adopted** — merged to `main` as [issue #168](https://github.com/seiya/atmofab/issues/168)'s [PR #193](https://github.com/seiya/atmofab/pull/193) on 2026-09-07, awaiting the billed A/B that `:247`'s equal-or-better rule requires, so this document's Z1 entry below is deliberately not marked LANDED; **Z3's implementation exists on the same terms** — merged to `main` as [issue #169](https://github.com/seiya/atmofab/issues/169)'s PR-2, awaiting its own billed A/B, and likewise not marked LANDED. The same issue's PR-3 puts the `infrastructure` harness self-test's `Generate` on the pure leaf as well (bundle `1.1.0`'s `runner` role), which closes the migration scope Z2 left open; it too awaits its billed run. **Z4's implementation exists on the same terms** — [issue #171](https://github.com/seiya/atmofab/issues/171)'s [PR #218](https://github.com/seiya/atmofab/pull/218) (2026-09-12) and [PR #219](https://github.com/seiya/atmofab/pull/219) (2026-09-13), recorded in `z4_enforcement_retirement.md` — and it landed AHEAD of the Z1 / Z3 adoption evidence by the operator's decision (Option B, issue #171's 2026-09-12 comments): one billed run after PR #219 is the adoption evidence for Z1 and Z3 and the completion evidence for Z4 at once, so none of the three is marked LANDED / ADOPTED here until that run is recorded. Z5-Z6 are not implemented. This document defines the target architecture the implementation converges to, compares it with the current implementation, and records the adoption decision (evolve in place / reimplement). It extends, and does not replace, `workflow_scaling_redesign.md` (R1-R6); overlaps are cross-referenced.
...
- The model has **no gate invocations and no repository write path**. Context (spec bodies, IR, dependency interfaces, exemplar, findings) is inlined by the host in a fixed, byte-stable order by default; artifacts come back as typed fields. Claude enforces the resulting closed context with no filesystem or shell tools; Codex uses a read-only sandbox plus structured output, so its equivalent is an approximation rather than strict tool-free isolation.
...
The leaf execution model until Z4 was an **agentic CLI session** (`claude -p` / `codex exec`) holding a shell inside the repository: it force-read contract documents at cold start, invoked gates itself, and wrote artifacts with file tools. Everything in the following inventory existed to make that actor safe and observable, had no counterpart under A2/A3, and was retired in Z4 ([issue #171](https://github.com/seiya/atmofab/issues/171); `z4_enforcement_retirement.md` records what was kept and why):
...
Measured consequences of that model (2026-07 baseline, measured before Z1): 96-97% of node wall time in LLM leaves, 89-90% of output tokens extended thinking, 35-50 KB cold force-reads per leaf with no shared prompt prefix, 43-62 min and 360-540k output tokens per certified node.
...
Each item is independently landable and gated by the standard decision criteria (billed E2E `aggregate_verdict=pass`; timing-audit cost verification; correctness-reducing changes rejected). An item that adds no executable path to the workflow — a contract, schema, or validator with no producer and no caller in the running phases (Z0) — is exempt from the billed E2E; that criterion attaches to the implementation-bearing items, which is where a behavior change can appear.

- **Z0. Codegen bundle and optimization-boundary contract. (LANDED 2026-07-14 — canonical: `docs/workflow/CODEGEN_BUNDLE_CONTRACT.md`, `spec/schema/generate/codegen_bundle.schema.json`, `spec/schema/generate/harness_capabilities.schema.json`, `tools/codegen_bundle.py`; record: `deterministic_followups.md` "Z0". The contract was pinned at `bundle_schema_version` 1.0.0 and is at 1.1.0 since issue #169 added the `runner` role; the `pure` Generate executor produces the bundle and `validate_bundle` is wired as the post-generate gate under Z2, below.)** Define and validate `CodegenBundle` before changing the Generate executor. The bundle contains `files[]` (`logical_path`, `role`, `language`, `content`), external `entrypoints`, a structured `target_lowering_plan`, `capability_requirements`, and `state_bindings`. It permits multiple compilation units, private helper procedures, internal modules, and target-specific execution algorithms. Every `logical_path` is normalized, relative, confined to the assembled source root, and unique within the bundle. The bundle forbids arbitrary shell commands and build scripts; the target backend derives the build graph from file roles and declared capabilities. Host rendering is limited to mechanical contract-boundary glue. Define optimization-unit membership and the versioned harness capability ABI in the same contract so semantic-node boundaries do not become mandatory optimization boundaries.
- **Z1. Pure-function stage pilot — the `compile` pair. (IMPLEMENTED, NOT YET ADOPTED — [issue #168](https://github.com/seiya/atmofab/issues/168)'s [PR #193](https://github.com/seiya/atmofab/pull/193), merged 2026-09-07; canonical: `docs/ORCHESTRATION.md` §pure-function leaf, `docs/workflow/phases/phase_01_compile.md` §1-1 / §1-2, `tools/prompt_templates/pure_compile_{generate,verify}.txt`; record: PR #193 + issue #168. **The pilot took `compile.verify` with `compile.generate`** — the item's title named only the producer, and the pair migrated together, so `LLM_LEAF_SUBSTEPS − PURE_CAPABLE_SUBSTEPS` lost both at once. The producer's THIRD exit (`compile_declared_fail`) and the pair's two recovery paths are `docs/ORCHESTRATION.md` §pure-function leaf; the pair was warm-recovery-only until [issue #209](https://github.com/seiya/atmofab/issues/209) made the cold path a repair turn carrying the findings and the prior document.)** Host inlines spec bodies + catalog + dependency IRs; the model returns `spec.ir.yaml` + `ir_meta.json` as structured output; the host writes them and runs the existing `Compile.static` gates; findings append as repair turns. Smallest surface (bounded inputs/outputs, no MCP use), measurable against the current baseline.
- **Z2. Host-mediated `generate.generate` + `generate.verify`. (LANDED 2026-07-16 — canonical: `docs/workflow/phases/phase_02_generate.md` (Generate-executor), `docs/workflow/CODEGEN_BUNDLE_CONTRACT.md`, `tools/pure_leaf.py`; record: `deterministic_followups.md` "Z2". The `pure` `generate-executor` runs the migrated `(generate, generate)` / `(generate, verify)` leaves as pure-function leaves on an `M3c` node; Claude uses strict tool-free isolation and Codex uses a sandboxed structured-output approximation. ADOPTED 2026-07-18 — the billed A/B (flux + `shallow_water2d` P arms) passed at byte-equal certified-outcome accuracy, and `pure` became the default at the adoption commit; **legacy generate execution was then REMOVED at M-F** (the `--generate-executor` flag + `ATMOFAB_GENERATE_EXECUTOR` env deleted, `pure` hardcoded, and a resume of a non-`pure` record rejected fail-closed with `generate_executor_legacy_removed`; record: `deterministic_followups.md` "Z2 adoption" / "M-F"). Issue #169's PR-3 closed the residual this entry used to record: the `infrastructure` harness self-test, the last node whose `generate` leaves ran the shared **agentic** loop, became bundle `1.1.0`'s `harness` shape and runs pure. Until Z4 what remained agentic was an operator's own choice — an `llm.yaml` entry whose `capabilities:` dropped `pure` — plus the fail-safe for a node with no bundle shape at all (`Conductor._bundle_shape` -> None), of which the spec catalog holds none; Z4 ([issue #171](https://github.com/seiya/atmofab/issues/171)) deleted both — such a configuration entry is refused at load, and such a node fails closed (`node_has_no_bundle_shape`). A node is still recorded `generate_executor=pure` (a provenance stamp), not a selectable executor.)** The host assembles the optimization-unit IRs, dependency facts, exemplar, target profile, and harness capability ABI. `generate.generate` returns one complete `CodegenBundle`; it does not return algorithm-step fragments for conductor concatenation. The host writes the declared files, deterministically assembles boundary glue and the build graph, and runs the existing lint/syntax/static gates. `generate.verify` reviews the bundle and assembled-source contract in an independent conversation. Closed-context execution is the default; bounded named-artifact reads are permitted only when the context-size policy requires them. This is the dominant-cost leaf and is expected to capture most of the prefix-caching benefit while retaining internal code-organization and hardware-optimization freedom.
- **Z3. Pure-function `validate.judge` (advisory reviewer). (IMPLEMENTED, NOT YET ADOPTED — issue #169 PR-2; canonical: `docs/workflow/phases/phase_04_validate.md` §4-2 and §4-2-1, `tools/raw_evidence_excerpt.py`, `tools/prompt_templates/pure_validate_judge.txt`. The excerpting policy is versioned by `RAW_EXCERPT_POLICY_VERSION` and stamped into each `semantic_review.json`. The leaf returns `{decision, findings, notes}` and the host writes the file, so a `--stage pre_judge` violation in it is a HOST-authored defect that terminalizes rather than warm-resuming the judge. With it, `LLM_LEAF_SUBSTEPS == PURE_CAPABLE_SUBSTEPS`: no LLM leaf holds tools.)** Host inlines evidence excerpts under an explicit excerpting policy; R2 already removed verdict authority. Retires the judge-side gate wiring.
- **Z4. Enforcement retirement per migrated stage. (IMPLEMENTED, NOT YET ADOPTED — [issue #171](https://github.com/seiya/atmofab/issues/171)'s [PR #218](https://github.com/seiya/atmofab/pull/218) and [PR #219](https://github.com/seiya/atmofab/pull/219); canonical: `docs/ORCHESTRATION.md` §pure-function leaf, `z4_enforcement_retirement.md`; record: issue #171 + the two pull requests. Two departures from the item as written: the deletion landed AHEAD of Z1's and Z3's adoption evidence (Option B, recorded in the note, with the ground and the cost), and `bwrap` did NOT move to build/execute — it stays as the wrapper of a pure CLI leaf, read-only, on both CLI providers, and putting it on build/execute (A8) is separate work. Not per stage either: the agentic leaf model was deleted whole once every LLM substep ran pure, because the complex bounded the actor rather than a stage.)** For each stage moved in Z1-Z3, delete its capability/manifest/hook/preflight surface and the leaf-facing contract text; `bwrap` remains solely on build/execute. Gateway prompt/response retention, response-schema validation, truncation classification, and audit records replace the retired authoring-side controls. Each deletion lands with the migration that makes it dead, never ahead of it. Any agentic fallback is an operator-selected diagnostic path for an unclassifiable failure, not an automatic normal-path retry.
...
| Z3 | Reduces evidence-reading and judge-side orchestration cost with low certification risk because the judge is advisory. | The excerpt policy is versioned, complete for routing, and supports bounded named-artifact reads for exceptional cases. **The bounded-read half was NOT taken** (issue #169): the excerpt is closed by the host, and a judge that could ask for a named artifact would be a leaf holding a tool again, which is the property this item exists to remove. If an excerpt proves incomplete the answer is the next policy version, not a read. | Incomplete excerpts can reduce semantic finding and repair-routing quality even though they cannot change the deterministic verdict. |
| Z4 | Deletes enforcement and contract complexity that exists only because an authoring leaf holds repository tools. Reduces maintenance burden and prompt size. | Gateway logging, schema validation, truncation handling, and stage-specific migration coverage are operational before deletion. | Provides limited direct runtime savings. Premature deletion weakens auditability or removes the only diagnostic path for novel failures. (As landed: the diagnostic path for a novel failure is the `escalate` diagnostician on the pure transport, issue #169 — not an agentic fallback, which no longer exists.) |
