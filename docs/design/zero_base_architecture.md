...
- **Z1. Pure-function stage pilot — the `compile` pair. (IMPLEMENTED, NOT YET ADOPTED — [issue #168](https://github.com/seiya/atmofab/issues/168)'s [PR #193](https://github.com/seiya/atmofab/pull/193), merged 2026-09-07; canonical: `docs/ORCHESTRATION.md` §pure-function leaf, `docs/workflow/phases/phase_01_compile.md` §1-1 / §1-2, `tools/prompt_templates/pure_compile_{generate,verify}.txt`; record: PR #193 + issue #168. **The pilot took `compile.verify` with `compile.generate`** — the item's title named only the producer, and the pair migrated together, so `LLM_LEAF_SUBSTEPS − PURE_CAPABLE_SUBSTEPS` lost both at once. `deterministic_followups.md` §"Z4 boundary" still lists the `compile.verify` agentic leaf as Z4 scope; that document is FROZEN (issue #181) and is not corrected in place — this entry is the current statement. The producer's THIRD exit (`compile_declared_fail`) and the pair's warm-only recovery path are `docs/ORCHESTRATION.md` §pure-function leaf; the HTTP half of that limit is [issue #209](https://github.com/seiya/atmofab/issues/209).)** Host inlines spec bodies + catalog + dependency IRs; the model returns `spec.ir.yaml` + `ir_meta.json` as structured output; the host writes them and runs the existing `Compile.static` gates; findings append as repair turns. Smallest surface (bounded inputs/outputs, no MCP use), measurable against the current baseline.
