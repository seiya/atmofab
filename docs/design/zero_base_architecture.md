...
- **Z2. Host-mediated `generate.generate` + `generate.verify`. (LANDED 2026-07-16 — canonical: `docs/workflow/phases/phase_02_generate.md` (Generate-executor), `docs/workflow/CODEGEN_BUNDLE_CONTRACT.md`, `tools/pure_leaf.py`; record: `deterministic_followups.md` "Z2". The `pure` `generate-executor` runs the migrated `(generate, generate)` / `(generate, verify)` leaves as pure-function leaves on an `M3c` node; Claude uses strict tool-free isolation and Codex uses a sandboxed structured-output approximation. ADOPTED 2026-07-18 — the billed A/B (flux + `shallow_water2d` P arms) passed at byte-equal certified-outcome accuracy, and `pure` became the default at the adoption commit; **legacy generate execution was then REMOVED at M-F** (the `--generate-executor` flag + `ATMOFAB_GENERATE_EXECUTOR` env deleted, `pure` hardcoded, and a resume of a non-`pure` record rejected fail-closed with `generate_executor_legacy_removed`; record: `deterministic_followups.md` "Z2 adoption" / "M-F"). Issue #169's PR-3 closed the residual this entry used to record: the `infrastructure` harness self-test, the last node whose `generate` leaves ran the shared **agentic** loop, became bundle `1.1.0`'s `harness` shape and runs pure. What remains agentic is an operator's own choice — an `llm.yaml` entry whose `capabilities:` drops `pure` — plus the fail-safe for a node with no bundle shape at all (`Conductor._bundle_shape` -> None), of which the spec catalog holds none. Such a node is still recorded `generate_executor=pure` (a provenance stamp), not a selectable executor.)** The host assembles the optimization-unit IRs, dependency facts, exemplar, target profile, and harness capability ABI. `generate.generate` returns one complete `CodegenBundle`; it does not return algorithm-step fragments for conductor concatenation. The host writes the declared files, deterministically assembles boundary glue and the build graph, and runs the existing lint/syntax/static gates. `generate.verify` reviews the bundle and assembled-source contract in an independent conversation. Closed-context execution is the default; bounded named-artifact reads are permitted only when the context-size policy requires them. This is the dominant-cost leaf and is expected to capture most of the prefix-caching benefit while retaining internal code-organization and hardware-optimization freedom.
