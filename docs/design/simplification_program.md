...
4. #171 Z4: the authoring-side enforcement complex deleted once no leaf holds a shell; blocked on 1 and 2. Implemented in PR #218 and PR #219 (2026-09-12 / 13), ahead of the billed evidence for 1 and 2 by the operator's decision; landed 2026-09-16 on the completion run `orch_20260916T081200Z_5139f6c9`, which is also 1's and 2's adoption evidence; `z4_enforcement_retirement.md` is the record. The read-only sandbox profile and the codex home / probe stay (premise 2); the codex feature cache did NOT stay, although the first version of this line said it would — its only reader was the codex leaf hook, which the retirement deleted, and a cache nobody reads is not kept by premise 2. (The profile BUILDER is `orchestration_runtime.build_readonly_bwrap_profile`, the pure leaf's shared path and since Z4 the only one. `workflow_conductor._readonly_sandbox_profile`, the diagnostician's in-process copy of it, is gone — #169's PR-1 gave the diagnostician a recorded launch like every other leaf's.)
