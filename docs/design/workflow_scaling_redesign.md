...
- **Incremental recertification** — content-hash-based invalidation over (spec, IR, dependency closure, harness version, target profile); unchanged nodes are never re-run (generalization of the existing ready-skip). A one-component change re-runs only its dependency-affected closure.
  ...
  - **Landed (R6 proper, closure-source half, 2026-09-04, issue #153)**: content-hash invalidation for the DEPENDENCY-CLOSURE input. `direct dependency execution readiness` now also requires a certified binary's recorded `sha256` for every dependency source it was compiled against (`binary_meta.json#dependency_check.closure_bindings[]`) to equal the one a build would stage today, so a dependency regenerated *within* one `spec_version` invalidates its consumers. Canonical: `docs/ORCHESTRATION.md` §13b and `deterministic_followups.md` "Issue #153". What R6 proper still lacks: hashing of the `spec` / IR input and of the target profile — an edit that moves neither a node_key nor a dependency source is still invisible, which is what keeps the respec discipline load-bearing. Issue #153 PR-2 narrowed what that discipline has to carry UNVERIFIED on a `component`: its published ABI is now pinned §5.1 ≡ IR ≡ SOURCE, so an ABI change can no longer come from a REGENERATION alone, where before it could. (Not "cannot happen without a `spec_version` bump": editing §5.1 in place moves neither a node_key nor a dependency source, so it is exactly the edit the sentence above says freshness still cannot see.)
