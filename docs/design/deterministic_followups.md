...
**Gate.** Always-on — the former opt-in env flags `METFORGE_CONDUCTOR_REUSE_SLIM_PROMPT` and
`METFORGE_CONDUCTOR_REUSE_RESUME` were removed (warm resume + slim are now the default). Slim applies
...
`METFORGE_CONDUCTOR_REUSE_RESUME` / `METFORGE_CONDUCTOR_REUSE_SLIM_PROMPT` and their
...
- `tools/run_workflow.py` — the `--generate-executor {legacy,pure}` flag (env `METFORGE_GENERATE_EXECUTOR`), persisted once
...
`METFORGE_GENERATE_EXECUTOR` / default) now defaults to `pure`, as does `run_conductor`'s env fallback (reached only
...
**What was removed.**
- The `--generate-executor {legacy,pure}` CLI flag and the `METFORGE_GENERATE_EXECUTOR` env var (`run_workflow.py`).
