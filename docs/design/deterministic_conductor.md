...
`ATMOFAB_CONDUCTOR_REUSE_RESUME` / `ATMOFAB_CONDUCTOR_REUSE_SLIM_PROMPT` were removed); it falls back
...
session transcript) and records `sandbox_enforced: true`. `spawn_leaf`
