...
| `--request-json-file` | one required | path to a file holding the same payload. **The conductor always uses this form** (no size branch, so there is no "it happened to fit" regression). It writes `workspace/orchestrations/<oid>/launches/<child_agent_run_id>.request.input.json` and keeps it as evidence even when this call fails |
...
| `--agent-run-json-file` | one required | path to a file holding the same payload. **The conductor always uses this form**; it writes `workspace/orchestrations/<oid>/launches/<agent_run_id>.agent_run.input.json` and keeps it as evidence |
