...
| `dependency_readiness.detail.pipeline_ref_verified` | `generate` onward | also carries the closure-binding comparison (`docs/ORCHESTRATION.md` §13b) |
...
**within-mark consistency**: `mark-dependency-readiness` judges each dependency artifact through one primitive (`_verify_dep_stage_detail`) and hashes the file that primitive selected, in a single `_compute_dep_readiness_and_fingerprint` pass, so the readiness booleans and the `dep_set_fingerprint` are derived from the same selection of files in one pass rather than from two separate walks. This closes the within-mark TOCTOU window in which a verification walk and a fingerprint walk could select or observe different artifacts; what remains is the interval between the primitive's read and the hash's read of the same path inside that pass. Because `_dependency_set_fingerprint` (for the gate) also uses the same walker, the same on-disk state always produces the same hash.
...
| `pipeline_ref` | under `workspace/pipelines/<kind>__<id>__<version>/`, select the "latest pipeline_id directory" (`_latest_pipeline_dir`), and the `verification_status == "pass"` of the latest `binary/*/binary_meta.json` in that pipeline — **plus** (R6 proper, closure-source half) that binary's `dependency_check.closure_bindings[]` recording, per closure node, the `model_source_sha256` a build would stage for it today (`_dependency_binding_freshness`). A binary with no `closure_bindings` key and a non-empty closure is legacy and fails closed; an empty closure (a leaf) records `[]` and passes. See `docs/ORCHESTRATION.md` §13b |
