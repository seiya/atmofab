...
- leaf node (computed is trivial-true), or an existing record that is a leaf's (`certified_deps == []`, e.g. a `spec_ref` since repointed to a non-leaf) → overwrite with the recompute (idempotent).
