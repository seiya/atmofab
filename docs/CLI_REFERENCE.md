...
- `deps.yaml` exists under `orchestration_meta.spec_ref` and `dependencies.components` / `dependencies.profiles` (and the optional `dependencies.infrastructure`) are all empty → all flags `true` (vacuous truth; for a leaf node)
...
- `reason=deps_yaml_malformed_schema`: `deps.yaml` can be parsed but the schema is malformed (the level directly under `dependencies` is missing a required key `{components, profiles}` or carries an unknown key beyond the optional `infrastructure`, not a list type, a missing `*_id` in each entry, a non-string `version_constraint`, a path-traversal token, etc.).
