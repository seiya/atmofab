...
**Certification stamp (`status=pass`, `compile` / `generate` / `build`).** After the payload validation and **before** the step_result file is written, the runtime stamps the phase's certification into its stage meta (`ir_meta.json` / `source_meta.json` / `binary_meta.json`): `artifact_hashes`, the sha256 of every `required_outputs` entry except that meta itself and the audit-log basenames (`command_log.jsonl`, `stdout.log`, `stderr.log`, `compile.stdout.log`, `compile.stderr.log` — Validate.execute APPENDS to Build's command log, so hashing it would make every Validate attempt read as a tampered Build), plus, for `generate` and `build`, `source_ir_id` taken from this orchestration's `compile` reservation (on `build` that OVERWRITES the value `_build_inproc` authored, which the child-window strip below has just erased — the reservation is what this orchestration holds the compile phase to, so an artifact cannot claim a lineage the run did not reserve). This is the single host-side writer of those keys: both the pure path (the host authors the meta) and the agentic path, which Z4 ([issue #171](https://github.com/seiya/atmofab/issues/171)) deleted, pass through here, and the child windows are all closed by the `child_finished` phase gate above. A stamp that cannot be taken (a declared deliverable absent, no meta or more than one in `required_outputs`, no compile reservation for a `generate` or a `build`) raises — **fail closed, unlike the WARN-only checkpoint update below**: an unstamped meta is indistinguishable from one whose author skipped the stamp, and `check-phase-certified` refuses both, so failing open would silently uncertify the phase forever. What the stamp pins is read back by `check-phase-certified` (the skip decision) and by the artifacts a later run stands on. A NON-passing `status` does the inverse: it STRIPS `artifact_hashes` / `source_ir_id` from that stage meta. On the agentic path the meta sat inside the verify leaf's write_root, so a leaf could write those keys itself (that path is deleted in Z4, and a pure leaf authors no file at all); while the phase passes that bought nothing, since the host overwrites them with its own measurement of the same files, but a phase ending non-pass would otherwise leave a meta claiming a certification no host issued — which is still the reason the keys are erased, because the host itself stamps them before it knows the phase's outcome. The stage meta is also stripped at the CHILD's own terminalization (`record-agent-run`), for whatever stage meta that child actually changed. That is the half that covers a phase fail-closing WITHOUT reaching `write-step-result` at all (a leaf transport error, a validate gate failure): `generate.verify`'s write_root IS `source_meta.json`, so that leaf can author the keys with perfectly correct values, and the phase would then read as certified on a later run although it never passed. After the two strips, the keys exist only when a PASSING `write-step-result` put them there — which runs when every child window is already closed.
