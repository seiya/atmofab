...
- `Compile` transcribes this published `operation_id` list into the `IR`'s `public_api.published_operations`, pinned == §5 by set equality (`_validate_published_surface`, `Compile.static`). That name pin is the source of truth a consumer's `Compile` is shown (the `dependency_surface.json` catalog) and that `_validate_component_generated_surface` pins the generated `<spec_id>__` symbol set against — so a component's published op name cannot drift between spec, IR, generated source, and a consumer's dependency call.
- **The subsection `### 5.1` below is required on a component too** (issue #153, 2026-09-04). The rules are the ones the `infrastructure` §5.1 states, and one gate over both kinds enforces them — see §"5.1 Canonical interface block" under the `infrastructure` spec shape below and read it as normative here. Until that date a component §5 carried NAMES ONLY and `signatures` / `module_parameters` were forbidden keys, on the reasoning that the argument ABI was better derived from the certified source at `Build`. What that produced instead: the ABI a component publishes was whatever the generation happened to emit, so re-generating one `spec_version` republished a different interface, and one component's six certified sources carried six different argument lists — two of them from the same `IR`. A consumer certified against the old one is then linked against an interface that has moved, which is only discovered at a downstream node's build. A component's `spec_version` is therefore what carries an ABI change, and §5.1 is what makes the ABI a statement of the spec rather than a property of the last generation.
