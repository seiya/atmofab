...
- `Compile` transcribes this published `operation_id` list into the `IR`'s `public_api.published_operations`, pinned == §5 by set equality (`_validate_published_surface`, `Compile.static`). That name pin is the source of truth a consumer's `Compile` is shown (the `dependency_surface.json` catalog) and that `_validate_component_generated_surface` pins the generated `<spec_id>__` definitions it scans against — so a component's published op name cannot drift between spec, IR, generated source, and a consumer's dependency call.
- **The subsection `### 5.1` below is required on a component too** (issue #153, 2026-09-04). The rules are the ones the `infrastructure` §5.1 states, and one gate over both kinds enforces them — see §"5.1 Canonical interface block" under the `infrastructure` spec shape below and read it as normative here. Until that date a component §5 carried NAMES ONLY and `signatures` / `module_parameters` were forbidden keys, on the reasoning that the argument ABI was better derived from the certified source at `Build`. What that produced instead: the ABI a component publishes was whatever the generation happened to emit, so re-generating one `spec_version` republished a different interface: one component's source trees under a single `spec_version` carry **six distinct argument lists**, two of them produced from the same `IR`. (The exact figure is six distinct LISTS; the source trees are more numerous, and the four the certified-binary selection would pick carry four of the six.) A consumer certified against the old one is then linked against an interface that has moved, which is only discovered at a downstream node's build. A component's `spec_version` is therefore what carries an ABI change, and §5.1 is what makes the ABI a statement of the spec rather than a property of the last generation.
...
- The subsection `### 5.1` below is required. The section numbers are load-bearing — the `Compile` gate reads the published surface out of `## 5.` and `### 5.1` by number — so a harness that renumbers its sections fails to certify.
  ...
  **5.1 Canonical interface block**
  - Required statement: the exact published surface as **one** fenced code block — a machine-readable, **language-neutral structured** signature block (a YAML mapping with `module_parameters` / `types` / `interfaces` / `procedures`) giving every published type and operation signature: for each argument, result, and derived-type component its `name`, neutral `type` (`real` / `integer` / `logical` / `string` / `derived` / `procedure`), `rank`, (for an argument) `intent`, and — for an EXPLICIT-SHAPE array argument — its `dims`, the list of bound expressions written in terms of the operation's own arguments and module parameters (`dims: ['nx']`, `dims: ['nx - 1']`, `dims: [ncomp, nx, ny]`); plus the value-pinned `module_parameters` the signatures reference. The vocabulary is neutral throughout — write a `string` `spec.len` as a **neutral token**: `deferred` (dynamic length) / `assumed` (caller-known length) / a fixed decimal width / a symbol identifier, never the Fortran `:` / `*`; write a `module_parameters` kind value as `float64` / `float32`, never the Fortran `real64` / `real32` (a Fortran expression such as `selected_real_kind(...)` has no neutral form and is rejected). `deferred` / `assumed` are reserved (case-insensitive), so a length symbol must not be named that. An operation that takes a **procedure as an argument** (an integrator taking the tendency it integrates) declares that argument as `spec: {type: procedure, interface: <name>}` — a procedure-typed argument has `rank` 0, no `dims`, and **no `intent`** (a dummy procedure carries none) — and `<name>` must be an entry of the `interfaces` list: a **named procedure prototype** written in the same form as a `procedures` entry (`kind` / `name` / `args` / `result`), whose own arguments may not be procedure-typed. Every `interfaces` entry must be referenced by at least one argument (an unreferenced prototype is refused: it is a published surface nothing reads), no two names in one scope may coincide case-insensitively — the module's (module parameters, types, prototypes and procedures), each procedure's or prototype's (its own name, its arguments, and a result not named exactly as the procedure), and each type's (its components); a coincidence is a `Compile fail` — and a procedure-typed `spec` is admitted only on a `procedures` argument (never a result, a derived-type component, or a prototype's argument). The consuming node passes a procedure matching the prototype; the prototype itself is declared, never defined. The target language's binding (Fortran `real(dp)` / `character(len=:)` / `character(len=*)` / `type(…)` / `integer, parameter :: dp = real64` / `<spec_id>__` names, and for a rank: `dims` if the argument declares them, else assumed-shape `(:)`) is produced by the language backend (`tools/backends/language/fortran/signatures`), not authored here — so the published-surface contract is independent of the implementation language (atmofab targets Fortran / C / C++ / mixed). Because `Compile` is target-free, it renders §5.1 in EVERY language whose backend declares `signatures`, so a §5.1 must lower in each of them, whatever target the node is later built for: a shape one language has no lowering for is a `Compile fail` for every node, reported like a malformed block. Today that means the CUDA C++ refusals as well as the first language's — a name that is a C++ keyword (`new`, `this`, `class`, `default`, `int`, …) or a CUDA execution-space specifier, a `logical` with a `kind`, and a `kind` naming an integer-valued module parameter (`docs/backends/language/cuda_cpp/BUNDLE_BINDING.md` §2). The `Compile` leaf cannot edit §5.1, so the fix is the spec author's.
  - `Compile` transcribes this block into the `IR`'s `public_api.signatures` (each `{symbol, signature}`, the same structured form), **`public_api.interfaces`** (each `{name, signature}`, one per `interfaces` entry, the entry itself; the key is omitted when §5.1 declares no prototype, and an `IR` that carries a prototype §5.1 does not declare — or omits one it declares — is a `Compile fail`) **and `public_api.module_parameters`** (each `{name, base?, value}`, value included), which are the only carriers of the signatures, the prototypes and the module-parameter values to `Generate` (`Generate.generate` does not read `controlled_spec.md`; see `docs/workflow/phases/phase_02_generate.md` §2-1). The gates pin §5 ≡ §5.1 ≡ `IR` `public_api` at `Compile` (rendering the structured signatures to the target language for the comparison; a prototype is compared by name and by its argument set, and is not an operation §5 lists), and pin the generated model source against those signatures — and each prototype against the source's declared prototype of that name, which the source must declare and must not define — by the `Generate.gate` static check. A missing `### 5.1`, a missing fence, more than one fence inside §5.1, a block that is not a valid structured mapping, or a §5 ↔ §5.1 symbol-set mismatch is a `Compile fail`. Declare each `module_parameters` name **once** (case-insensitively — target-language identifiers may be case-insensitive, so `dp` and `DP` are the same parameter) — a repeated name is a `Compile fail` (it cannot be pinned coherently: the `Generate.gate` static check renders the full un-deduped list, so a diverging duplicate like `dp = float64` and `dp = float32` would demand contradictory declarations in the generated source).
