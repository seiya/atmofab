...
- `Compile` transcribes this published `operation_id` list into the `IR`'s `public_api.published_operations`, pinned == §5 by set equality (`_validate_published_surface`, `Compile.static`). That name pin is the source of truth a consumer's `Compile` is shown (the `dependency_surface.json` catalog) and that `_validate_component_generated_surface` pins the generated `<spec_id>__` subroutine set against — so a component's published op name cannot drift between spec, IR, generated source, and a consumer's dependency call.
