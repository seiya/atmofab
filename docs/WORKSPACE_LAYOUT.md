...
| `workspace/pipelines/.../<pipeline_id>/source/<source_id>/source_meta.json` | Generate | conductor (host-written; a leaf authors no file since Z4); `artifact_hashes` + `source_ir_id` stamped by `write-step-result` | Build / validator | `source_ir_id` (from the `compile` reservation) is this source's IR binding; `artifact_hashes` pins the generated sources. Both are read by `check-phase-certified` |
