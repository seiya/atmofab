...
Every terminal row a conductor leaf launch produces carries a `usage` field (also appended verbatim to `agent_runs.jsonl`, which is what `tools/audit_orchestration.py` reads). It is EITHER a measurement or an explicit statement that there is none — never an empty object that would read as zero cost. Every terminal row a `substep` produces carries one, whether or not it launched a leaf. The rows that carry none are the orchestration agent's own row and the `record-timeout` recovery path. The failure diagnostician used to be the third: it produced no `agent_runs.jsonl` row at all and its cost was emitted as a `diagnose_leaf_usage` event into the run log instead. Issue #169 gave it a child `agent_run_id`, so it carries an ordinary terminal row (`step=<the failed phase>`, `substep=diagnose`, `output_refs: []`, tombstoned in `reopen/superseded_runs.json` because it vouches for no `step_result.json`) with its `usage` on it, and its transcript is at `agents/<arid>/dialogs/leaf.stdout.log` like every other leaf's — one per escalate, rather than one fixed path per phase that a second escalate overwrote.
