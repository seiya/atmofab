...
| `workspace/pipelines/.../<pipeline_id>/runs/<run_id>/<node_key_safe>/verdict.json` | Validate/judge | conductor (host-written from the judge leaf's returned document; `workflow_conductor` authors it after the child window closes) | runtime / validator / upper node | |
