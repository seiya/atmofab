...
| `workspace/ir/.../<ir_id>/ir_meta.json` | Compile/generate / verify | conductor (host-written; there is no other path — a `pure-function leaf` holds no tool and the host writes every artifact from the document it returns, Z4 / [issue #171](https://github.com/seiya/atmofab/issues/171)); `artifact_hashes` stamped by `write-step-result` | runtime / validator | `verification_status` is assigned only when verify passes. `artifact_hashes` pins the sha256 of `spec.ir.yaml`; together they are what `check-phase-certified` reads. `output_hash` / `derivation_key` / `derivation_inputs` / `derivation_transformation` are stamped beside them ([issue #250](https://github.com/seiya/atmofab/issues/250); recorded, read by nothing until PR-2) |
...
| `workspace/pipelines/.../<pipeline_id>/source/<source_id>/source_meta.json` | Generate | conductor (host-written; a leaf authors no file since Z4); `artifact_hashes` + `source_ir_id` stamped by `write-step-result` | Build / validator | `source_ir_id` (from the `compile` reservation) is this source's IR binding; `artifact_hashes` pins the generated sources — the model, the checks module, the runner and the control file, whoever authored them (issue #250). Both are read by `check-phase-certified`. The derivation stamp (`output_hash` + the three `derivation_*` keys) sits beside them |
