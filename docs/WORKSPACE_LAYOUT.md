...
**Measured** ([issue #94's re-measurement](https://github.com/seiya/atmofab/issues/94#issuecomment-5795190195)): on a warm relaunch the top-level `usage` is always the turn's own; `modelUsage` and `total_cost_usd` are the turn's own on every recorded CLI version from 2.1.211 to 2.1.275 and the resumed session's running total on 2.1.278 and 2.1.280 (`orch_20260922T150103Z_fbfd2d76`: three turns of one session report 43,861 / 74,526 / 105,418 output tokens against a per-turn 43,861 / 30,665 / 30,892). `num_turns` is 1 in both cases, so it cannot tell them apart. Rows recorded before issue #281 on 2.1.278 and later still hold the running total on disk; `orchestration_diagnostics._sum_pure_attempt_usage` and `tools/audit_orchestration.py` overstate those runs, and `skills/workflow-timing-audit/scripts/attempt_cost_report.py` corrects them for its own figures.
