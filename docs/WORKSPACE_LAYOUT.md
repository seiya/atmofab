...
| `workspace/ir/.../<ir_id>/ir_meta.json` | Compile/generate / verify | conductor (host-written on the default `pure` path; substep agent (Edit/Write) on the residual agentic one); `artifact_hashes` stamped by `write-step-result` | runtime / validator | `verification_status` is assigned only when verify passes. `artifact_hashes` pins the sha256 of `spec.ir.yaml`; together they are what `check-phase-certified` reads |
...
| `workspace/pipelines/.../<pipeline_id>/source/<source_id>/source_meta.json` | Generate | substep agent (Edit/Write) (host-written on the `pure` path); `artifact_hashes` + `source_ir_id` stamped by `write-step-result` | Build / validator | `source_ir_id` (from the `compile` reservation) is this source's IR binding; `artifact_hashes` pins the generated sources. Both are read by `check-phase-certified` |
...
> **The leaf-facing half of this section is HISTORICAL since Z4 ([issue #171](https://github.com/seiya/atmofab/issues/171)).** It describes what an AGENTIC leaf had to do to write a temporary file — the literal `allowed_tmp_root` path, the `Write` tool rather than a redirect, and `output_manifest_write_guard` as what refused the alternatives. That hook is deleted with the leaf: a `pure-function leaf` writes no file at all, temporary or otherwise, and returns one JSON document the host writes from. What still uses the tmp root is the CONDUCTOR's own deterministic substeps and `run-gate`. The structural deletion of the manifest machinery this section is written around belongs to PR-2 of the same issue; this note is so the section does not read as current in the meantime.
