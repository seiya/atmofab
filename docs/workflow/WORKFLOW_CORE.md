...
- The `Generate.generate` / `Generate.verify` leaves run under the `pure` `generate-executor` — the only executor since `M-F` (legacy generate execution removed; `docs/GLOSSARY.md`). The value is recorded (always `pure`) in `orchestration_meta.json#invocation.generate_executor`, and a resume of a non-`pure` record is rejected fail-closed (`generate_executor_legacy_removed`). On an `M3c` node `Generate.generate` is a `pure-function leaf` returning one `CodegenBundle` that the host validates (`validate_bundle`) and writes, and `Generate.verify` is a `pure-function leaf` returning a verify verdict the host projects onto `source_meta.json`; the source directory additionally holds `codegen_bundle.json`, `bundle_meta.json`, `verdict_meta.json`. Claude uses strict tool-free isolation and Codex uses the sandboxed structured-output approximation. A residual node the pure producer cannot express (a node whose runner/Makefile are not host-rendered) runs the shared **agentic** leaves, which write the source and re-author `source_meta.json` directly. The deterministic `Generate.gate` substep (its lint / syntax / static checks) and the authoring rules below are identical either way. Canonical: `phases/phase_02_generate.md`.
...
- The phase doc (`docs/workflow/phases/phase_*.md`) is force-read **only for `Compile`** (`phase_01`'s IR schema is the contract the compile SKILL defers to). The `Generate` / `Validate` SKILLs are self-sufficient and cite their phase doc as canonical without force-reading it; `Generate.generate` / `Generate.verify` / `Validate.judge` instead force-read `docs/workflow/RUNNER_OUTPUT_CONTRACT.md` (the consolidated runner-output contract); the pure judge is handed its §1 and §3 inline instead. Canonical rationale: `docs/design/leaf_must_read_restructure.md`. `docs/WORKFLOW.md` is the entry point to the specification. For a `pure-function leaf` (both `Compile` LLM substeps, `Validate.judge`, and the `Generate.generate` / `Generate.verify` executor on an `M3c` node — since `Z3` that is every LLM leaf) this force-read does not apply: it reads no `SKILL` and no `skill_must_read_refs` — the host inlines the closed context into the launch prompt (`docs/GLOSSARY.md`, `docs/workflow/LAUNCH_PROMPT_REFERENCE.md`). A residual agentic leaf — a non-`M3c` `generate` node, or any leaf whose configured entry drops the `pure` capability — still force-reads as above.
