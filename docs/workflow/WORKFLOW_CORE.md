...
- `substep` refers to a lower execution unit decomposed from a `step`.
  ...
  - `Generate` has the 3 substeps `generate`, `gate`, and `verify` — `gate` is a deterministic conductor-run substep (no leaf) between `generate` and `verify` that runs three checks: a lint check, a syntax check (the MCP `run_syntax_check` compiler front-end gate, gfortran `-fsyntax-only`), and a static check (`validate_workspace_root` then `validate_pipeline_semantics --stage post_generate`, run only when the lint and syntax checks both pass); a lint, syntax, or static finding in the node's own source warm-resumes `generate` (a syntax finding the staged dependency closure causes instead fails closed — the leaf cannot write it), so the LLM `verify` is reached only on a deterministically-clean source.
