...
35. The `quality check` execution method must be consistent with the target profile's `toolchain.language` and `toolchain.build_system`. With `toolchain.build_system=make` and `toolchain.language=fortran` / `c` / `cpp` / `mixed` families, use `make_test` or `make_check`, and forbid substitution by `pytest`.
