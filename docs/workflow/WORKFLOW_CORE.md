...
- The phase doc (`docs/workflow/phases/phase_*.md`) is force-read **only for `Compile`** (`phase_01`'s IR schema is the contract the compile SKILL defers to). The `Generate` / `Validate` SKILLs are self-sufficient and cite their phase doc as canonical without force-reading it; `Generate.generate` / `Generate.verify` / `Validate.judge` instead force-read `docs/workflow/RUNNER_OUTPUT_CONTRACT.md` (the consolidated runner-output contract) — a statement about the residual agentic path, since no in-tree leaf takes it any more: the pure judge is handed its §1 and §3 inline, and the `harness` shape's two pure generate leaves are handed the whole document (issue #169). Canonical rationale: `docs/design/leaf_must_read_restructure.md`. `docs/WORKFLOW.md` is the entry point to the specification. For a `pure-function leaf` (both `Compile` LLM substeps, `Validate.judge`, and `Generate.generate` / `Generate.verify` on any node with a `CodegenBundle` shape — since issue #169 that is every LLM leaf of every catalog node) this force-read does not apply: it reads no `SKILL` and no `skill_must_read_refs` — the host inlines the closed context into the launch prompt (`docs/GLOSSARY.md`, `docs/workflow/LAUNCH_PROMPT_REFERENCE.md`). A residual agentic leaf — a `generate` node with no `CodegenBundle` shape, or any leaf whose configured entry drops the `pure` capability — still force-reads as above. Only the second of those is reachable today.
...
- The workflow completion condition is that, for the `all_nodes` set of the `dependency_graph.json` sidecar, `workspace/ir/<node_key_safe>/<ir_id>/` and `workspace/pipelines/<node_key_safe>/<pipeline_id>/` exist, and the `node_key` and `dependency_ref` of `lineage.json` match.
