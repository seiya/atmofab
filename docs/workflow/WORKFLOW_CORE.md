...
26. The canonical entrypoint for starting the workflow is `python3 tools/run_workflow.py <spec_ref> <until_phase> [--llm-config <path>]`. `<until_phase>` specifies one of `compile` / `generate` / `build` / `validate`; `--llm-config` selects the LLM of each phase / `substep` from a configuration file (default `configs/llm/claude.yaml`), and a `codex_cli` entry additionally requires an explicit `model:`. The `--llm` / `--agent-model` / `--llm-command` trio is deprecated and maps onto the shipped `configs/llm/<backend>.yaml`.
