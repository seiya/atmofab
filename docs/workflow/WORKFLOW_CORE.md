...
2. `Generate`: take the IR as input and generate the source of `model` and `runner`. (On an M3c physics node — a non-`infrastructure` node whose target toolchain the host renders a runner for; the runner is glue over the target's harness — the leaf authors `model` + `<spec_id>_checks.f90` and the `runner` is host-rendered by the conductor; see phase_02 §2-1.)
...
26. The canonical entrypoint for starting the workflow is `python3 tools/run_workflow.py <spec_ref> <until_phase> [--llm-config <path>]`. `<until_phase>` specifies one of `compile` / `generate` / `build` / `validate`; `--llm-config` selects the LLM of each phase / `substep` from a configuration file (default `./llm.yaml`, created by copying a sample from `docs/examples/`), and a `codex_cli` entry additionally requires an explicit `model:`. The configuration file is the only thing that says what a leaf launches.
...
29. Just before launching a child `agent`, re-run the execution platform's live check, and confirm the satisfaction of `multi_agent=true` and the launchability of the child `agent`. `multi_agent` is exempt for `backend=codex` alone, where it is advisory diagnostics: with `backend=codex` the conductor launches independent Codex CLI processes and uses no Codex subagent, so the child `agent`'s launchability is carried by the Codex capability checks instead (canonical: `docs/ORCHESTRATION.md` §preflight and launch control).
...
35. The `quality check` execution method must be consistent with the target profile's `toolchain.language` and `toolchain.build_system`. With `toolchain.build_system=make` and a compiled `toolchain.language` (its language backend declares `COMPILED`, `registry.is_compiled_language`), use `make_test` or `make_check`, and forbid substitution by `pytest`.
...
- `source_meta.json` requires the common keys above. Lint is no longer recorded in `source_meta.lint_command_ref` — it is the deterministic conductor-run `Generate.gate` lint check, certified by `post_generate` against the host-authored `<pipeline_root>/lint_evidence/<source_id>.json`.
...
- output: `source/<source_id>/src/`, `source_meta.json`, `gate_meta.json` (the single conductor-authored `Generate.gate` deliverable; see next bullets)
- The `Compile.generate` / `Compile.verify` leaves are `pure-function leaf`s on EVERY node ([issue #168](https://github.com/seiya/atmofab/issues/168)): the host inlines this phase's `spec` documents, `phase_01_compile.md` in full, and the IR reference forms (no target: the IR is target-free since R4-a PR-3, issue #284), and the leaf returns one JSON document — `{ir, last_fail_reason}` from the producer, a verify verdict from the reviewer — which the host validates and writes as `spec.ir.yaml` / `ir_meta.json`. The per-attempt records are `compile_generate_meta.json` / `compile_verify_meta.json` beside them. There is no node-shape condition on this pair; the deterministic `Compile.static` gate is unchanged and is still the only semantic gate of the phase. Canonical: `phases/phase_01_compile.md`.
- The `Generate.generate` / `Generate.verify` leaves run under the `pure` `generate-executor` — the only executor since `M-F` (legacy generate execution removed; `docs/GLOSSARY.md`). The value is recorded (always `pure`) in `orchestration_meta.json#invocation.generate_executor`, and a resume of a non-`pure` record is rejected fail-closed (`generate_executor_legacy_removed`). On any node with a `CodegenBundle` shape — `m3c`, or `harness` since issue #169 — `Generate.generate` is a `pure-function leaf` returning one `CodegenBundle` that the host validates (`validate_bundle`) and writes, and `Generate.verify` is a `pure-function leaf` returning a verify verdict the host projects onto `source_meta.json`; the source directory additionally holds `codegen_bundle.json`, `bundle_meta.json`, `verdict_meta.json`. Claude uses strict tool-free isolation and Codex runs inside a read-only sandbox. A residual node the pure producer cannot express (one with no `CodegenBundle` shape; the catalog holds none) fails closed (`node_has_no_bundle_shape`); until Z4 (issue #171) it ran the shared **agentic** leaves, which wrote the source and re-authored `source_meta.json` directly. The deterministic `Generate.gate` substep (its lint / syntax / static checks) and the authoring rules below are identical either way. Canonical: `phases/phase_02_generate.md`.
- Before `Generate` completes, the conductor's deterministic `Generate.gate` substep runs three checkers in-process and writes ONE `source/<source_id>/gate_meta.json` (the freshness-gated deliverable, with a `checkers` object holding `lint` / `syntax` / `static` sub-sections, a `failure_categories` list in canonical order `syntax_error` → `lint_findings` → static-family, and a composed `failure_excerpt`). The lint check always runs (MCP `run_linter` consistent with the target profile's `toolchain.language`, host-authored lint evidence `<pipeline_root>/lint_evidence/<source_id>.json`, each element with `command_id`, `command_log_ref`, and `preset`). The syntax check always runs (independent of lint): the MCP `run_syntax_check` compiler front-end gate (the target language's mandatory syntax-only stage, `bundle_facts.MANDATORY_SYNTAX_COMPILER`, with its `syntax_promotions`, over the node + staged dependency-closure sources; a language that declares no syntax stage is a transport `fail_closed`; optional `ATMOFAB_SYNTAX_COMPILERS` target-compiler stages are recorded `skipped` when their binary is not installed or their adapter reads another language), writing host-authored syntax evidence (`<pipeline_root>/syntax_evidence/<source_id>.json`, per-stage `compiler` / `status` / `command_id` / `command_log_ref`). The static check runs **only when the lint and syntax checks both pass** (`validate_workspace_root.py` then `validate_pipeline_semantics --stage post_generate` — the purely-static gates the verify leaf used to own); when skipped its section records `status="skipped"` / `skipped_reason="lint_or_syntax_failed"`. `post_generate` certifies the lint and syntax evidence (the language's mandatory stage must have passed). A gate content failure (a lint finding, a compiler finding in the node's own source, or a static violation) warm-resumes `generate` with route reason `gate_<cat1>+<cat2>+...`; a failing syntax stage the deterministic attribution runs trace to an unviable invocation (a `toolchain.standard` the compiler driver rejects) or to the staged dependency closure raises a transport `fail_closed` naming what to fix (no `gate_meta.json` written), and dominates a co-occurring lint content failure, since the leaf authors neither; non-fortran languages pass through. Otherwise `Generate.verify` runs as a pure LLM semantic pass that launches no validator gate. The leaf does not run `run_linter` / `run_syntax_check` or these validators.
...
- With `toolchain.build_system=make` and a compiled `toolchain.language` (its language backend declares `COMPILED`, `registry.is_compiled_language`), a `test` or `check` target must be defined in `source/<source_id>/src/Makefile`. On absence, it is `Generate fail`.

### 3. Build
- execution input: `source/<source_id>/src/`, the target profile (`spec/targets/<target_id>.yaml`)
...
- `Validate` has the `pre_judge`, `execute`, `judge`, and `post_judge` substeps. The deterministic `pre_judge` substep runs the dependency-DAG readiness check; the `execute` substep calls `run_program` via MCP to generate execution evidence; the `judge` substep finalizes the `verdict` by an LLM semantic check over the evidence the host inlines for it (including a bounded excerpt of `raw/` in place of the arrays themselves); the deterministic `post_judge` substep runs the `--stage pre_judge` gate and classifies violation severity.
...
- `skill_must_read_refs` is declared EMPTY on every launch. It was the forced-reading list a child `step agent` / `substep agent` was assembled for (`build_skill_must_read_refs`, deleted), and the field survives only as the explicit empty a pure request declares, which the launch validator checks.
- A leaf reads no document at all. Since Z4 ([issue #171](https://github.com/seiya/atmofab/issues/171)) every LLM substep is a `pure-function leaf`: the host inlines the documents that substep needs into its launch prompt and the leaf returns one JSON document. `docs/AGENT_CONTRACT.md` was the single common leaf must-read and is deleted with the transport that delivered it — the leaf-actionable invariants it carried are the subset of this document's §"Workflow common invariants", and the `<stage>_meta.json` key rules are §"Stage meta keys" above. **This document (`WORKFLOW_CORE.md`) and `docs/ORCHESTRATION.md` are orchestration/operator canonical**, as they were.
- No phase doc is force-read either. `phase_01_compile.md` was the one exception — `phase_01`'s IR schema, force-read by a compile leaf — and it now reaches both compile leaves the way every other document does: INLINED into the launch prompt as `phase_contract_document`, whole (`Conductor._build_pure_compile_context` / `_build_pure_compile_verify_context`).
