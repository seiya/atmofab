...
26. The canonical entrypoint for starting the workflow is `python3 tools/run_workflow.py <spec_ref> <until_phase> [--llm <codex|claude>]`. `<until_phase>` specifies one of `compile` / `generate` / `build` / `validate`; `--llm codex` additionally requires `--agent-model <explicit-model-slug>`.
