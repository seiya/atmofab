...
- `artifact_hashes` is host-written (at `write-step-result`, on a `pass`) and must not be authored here; leave any you find untouched.
...
`issue_severity` names the repair a finding calls for. On the agentic path the leaf records it in `ir_meta.json` next to `last_fail_reason`; on the pure path the leaf returns it in its verdict and the host projects both onto that file. The conductor acts on the value as `docs/workflow/WORKFLOW_CORE.md` states; the value does not grade the consequence the defect would have at `Generate` or `Validate`, and the size of the correction does not change it. One question selects the value: whether `Compile.generate`, re-run with the finding and the same `controlled_spec.md`, `tests.md`, `deps.yaml` and `spec/registry/spec_catalog.yaml` — this substep's verification inputs, listed in §I/O contract above — can resolve the finding. This is the axis `docs/workflow/phases/phase_02_generate.md` §2-2 §Severity of a finding applies to `Generate.verify`; `spec.ir.yaml` is the artifact under review here and an input there, so a thin lowering takes `minor` here and `major` there.
