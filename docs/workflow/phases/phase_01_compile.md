...
- Author `dependency.node_key` and `dependency.direct_deps[]` (each with `kind` + `operations`) — the HOST's directly-required set, which is `dependency_graph.json`'s `all_nodes` minus this node minus its `transitive_deps`. Read it there, not from `deps.yaml`: a `profile` entry in `deps.yaml` is not a node, and the `component` it selects are what the direct set holds (issue #175). A `component/` dep's `operations` MUST be non-empty (every `<dep_spec_id>__*` subroutine this node calls) — pinned by the deterministic gate `_validate_component_dep_operations` (V4c-i, below). No `infrastructure` node is in the set: the runner harness belongs to the target the node is built for, and the host adds it to every pipeline of the node (issue #284). Do NOT author `transitive_deps` / `all_nodes`: the conductor derives that closure/topo graph host-side into `<ir_ref>/dependency_graph.json` (a pure function of `deps.yaml` + `spec_catalog.yaml`).
...
- **V4c-i (component `operations` non-empty) — deterministic Compile gate.** Every `component/`
  ...
  requires the `use`/`call`, so a `pure` leaf cannot converge (retry budget exhausts). `problem`
