...
io_contract:
  ...
  #   corroborant states the state fact the guard leaves behind) — evaluated in a scope that covers the
  #   condition's: a `per_case: true` condition needs a `per_case: true` corroborant of that quantity, a
  #   `case: X` condition one with `case: X` or `per_case: true`, a suite-level condition takes either (a
  #   corroborant pinned to one case never stands for a condition that holds in every case). A pass test
  #   asserting only verdict.* is therefore no longer refused as degenerate: its judgment is conjoined
  #   with a host-evaluated predicate, never reduced to the runner's own verdict.overall.
