...
`issue_severity` names the repair a finding calls for; the leaf records it in `ir_meta.json` next to `last_fail_reason`. The conductor acts on the value as `docs/workflow/WORKFLOW_CORE.md` states; the value does not grade the consequence the defect would have at `Generate` or `Validate`, and the size of the correction does not change it. One question selects the value: whether `Compile.generate`, re-run with the finding and the same `controlled_spec.md`, `tests.md`, `deps.yaml` and `spec_catalog.yaml`, can resolve the finding. This is the axis `docs/workflow/phases/phase_02_generate.md` §2-2 §Severity of a finding applies to `Generate.verify`; `spec.ir.yaml` is the artifact under review here and an input there, so a thin lowering takes `minor` here and `major` there.
- `minor`: the defect lies in `spec.ir.yaml`, and `last_fail_reason` names a correction that keeps it faithful to `controlled_spec.md`, `tests.md`, `deps.yaml` and `spec_catalog.yaml`. Every finding against the invariant set above whose subject is the IR takes this value — a `case_id` `tests.md` requires and `case` lacks, a local operation lowered without the defining math `controlled_spec.md` carries, a declared `execution_mode` that contradicts the control structure `controlled_spec.md` describes, a `test_predicates` entry that mistranslates its `tests.md` prose, a `required_raw_variables` list that omits a recompute input, an `impl_defaults` plug-hole.
