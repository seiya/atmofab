...
algorithm:
  ...
  iteration_contract: {...}    # object. non-empty when execution_mode=iterative; conversely a loop counter + stop_condition here forbids any non-iterative mode
  update_semantics: {...}
...
1. **`columnwise`** — the algorithm processes the grid column by column; `steps[]` holds at least one `column_process` step. Decided before `iterative` because a column sweep is also a repeated body: the `column_process` step is what tells the two apart.
2. **`iterative`** — the algorithm repeats a body while advancing a loop counter (time index, iteration index) until a stop condition holds. **Any time marching over `n_step` is `iterative`**, never `sequence`, however few steps the body has. Branches inside the body do not change the mode. Author the loop in `iteration_contract` (loop counter + `stop_condition`) and describe it in `control_condition`.
...
**Scope: `algorithm.iteration_contract` describes the TOP-LEVEL loop only.** A step that converges internally (`step_kind: iterative_solve`) does not make the algorithm `iterative`, and its convergence criteria (tolerance, iteration cap, divergence policy) are lowered in that step's own fields / `invariants` under the V2 IR-self-sufficiency rule — NOT in `algorithm.iteration_contract`. An inner solve's loop written into `algorithm.iteration_contract` under `sequence` is a `Compile fail`, because that field states what the whole algorithm does.

#### Mode ↔ contract couplings
Each of these is enforced by the `--stage compile` gate and is a `Compile fail` routed to `Compile.generate`:
...
- `execution_mode` of `sequence` or `conditional` ⇒ `iteration_contract` carries no loop counter (`loop_variable` / `counter` / `index_variable`) together with `stop_condition`. That pair describes a repeated body, which those two modes deny. The remedy is to set `execution_mode: iterative` keeping the loop fields, or — when the algorithm truly runs once — to drop the loop-counter/`stop_condition` keys. `iterative` and `columnwise` both denote a repeated body, so a loop-shaped contract is expected under them and this rule does not apply. Under `sequence` / `conditional` a non-loop `iteration_contract` stays legal: `{}`, a negative declaration (`{applies: false, rationale: ...}`, `{kind: none, reason: ...}`, including one whose loop keys are present but `null`), a fixed stage composition, and the `case_loop` dispatch shape all pass.
- A non-empty `control_condition` under `execution_mode=sequence` is **legal**: that field also carries guard narration, and no converse rule is placed on it.
