...
- `python3 tools/validate_pipeline_semantics.py --stage compile --ir-ref workspace/ir/<node_key_safe>/<ir_id>/`, with `exit code 0` required. The result is recorded in `compile_static_meta.json`; on `fail` the substep fails before `Compile.verify` runs, so `verification_status=pass` is never reached on a structurally-invalid IR. (The `--stage compile` validator checks the internal-consistency / shape-grammar invariants; the spec-cross-reference invariants are the `Compile.verify` LLM responsibility — see the substep structure note.)
  ...
  - **toolchain backend** (`_validate_toolchain_backend_supported`, every non-`infrastructure` node): `impl_defaults.toolchain` declares `build_system` `make` and `language` `fortran`. State both explicitly (V6; the `post_generate` lint / syntax-evidence gates read `language`, and an absent one silently skips the Fortran syntax gate). They are also the defaults an absent key takes — that exists so this gate and the conductor read an IR identically, not as a licence to omit. Any other pair is a `Compile fail` routed to `Compile.generate`: the host-authored `src/Makefile` and `src/<spec_id>_runner.f90` exist for `make` + `fortran` only, so no other toolchain has a physical path, and the `controlled_spec` is language-neutral and pins none — a re-author repairs it. The `Build` stage keeps a `make`-only backstop (`_require_make_build_system`). The same gate rejects three SHAPES on **every** node, `infrastructure` included — these are not about which backend is supported but about the host's own readers disagreeing: a truthy non-mapping `impl_defaults` / `toolchain`; an explicitly-null `build_system` / `language` (the conductor coerces `null` to the default and host-authors `src/Makefile`, while `record-launch`'s line-scanning reader sees the literal `null`, concludes the leaf authors it, and pins it into the leaf's write set — leaving the file **double-owned**); and a value carrying leading or trailing whitespace (the conductor lower-cases but does not strip, while the backstop and that reader do). The pair comparison is case-insensitive but NOT trimmed, mirroring the conductor byte-for-byte.
