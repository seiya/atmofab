...
io_contract:
  ...
  primary_predicates:
    ...
    # A test carries one per quantity its conditions name (several conditions may share a name; a test never
    # carries none — the coverage gate refuses it). Every
  ...
  #   corroborant states the state fact the guard leaves behind). A pass test asserting only verdict.*
  #   is therefore no longer refused as degenerate: its judgment is conjoined with a host-evaluated
