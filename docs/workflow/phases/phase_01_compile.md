...
dependency:
  ...
  direct_deps:
    - node_key: "<spec_kind>/<spec_id>@<spec_version>"
      kind: "<component|problem>"   # never `profile` (not a node) and never `infrastructure`: the
                                     #   runner harness is the target's, not a dependency of the node
...
public_api:
  ...
  signatures:                             # copy §5.1 structured signatures, one {symbol, signature} each
    - symbol: "<spec_id>__<name>"         # every §5.1 op AND type (set equality with §5.1)
      signature:                          # structured MAPPING, NOT source text: a procedure
        ...
        args:                             #   name?, alloc?}}. Generate renders it to the target language
...
- Read the physics algorithm (A) of the `Controlled Spec`, deterministically expand the input conditions and `sweep` / `refinement` from `tests.md`, and generate the 4 sections `case` / `algorithm` / `io_contract` / `dependency` of `spec.ir.yaml`. The IR names no implementation target: the toolchain, hardware, parallel model and harness are the host's target profile (`spec/targets/<target_id>.yaml`, chosen per run after Compile), and the lowering onto them is decided at Generate (the bundle's `target_lowering_plan`). An `impl_defaults` section — which carried both until issue #284 — is refused by the host. For an `infrastructure` or `component` node additionally author the `public_api` section — the COMPLETE published surface controlled_spec §5 declares (every `operation_id`, incl. helper emitters/writers no test exercises directly, plus every published derived type) AND `public_api.signatures` (each `{symbol, signature}`) copying the §5.1 structured signatures AND — when §5.1 declares an `interfaces` list of named procedure prototypes (issue #266) — `public_api.interfaces` (each `{name, signature}`, the entry itself; omitted when §5.1 declares none) AND `public_api.module_parameters` (each `{name, base?, value}`) copying the §5.1 module_parameters value-included; the gate pins names == §5, `signatures` == §5.1, `interfaces` == §5.1 by name set and argument set, and `module_parameters` == §5.1 by value (V8). A `problem` node omits `public_api`.
- Author `dependency.node_key` and `dependency.direct_deps[]` (each with `kind` + `operations`) — the HOST's directly-required set, which is `dependency_graph.json`'s `all_nodes` minus this node minus its `transitive_deps`. Read it there, not from `deps.yaml`: a `profile` entry in `deps.yaml` is not a node, and the `component` it selects are what the direct set holds (issue #175). A `component/` dep's `operations` MUST be non-empty (every `<dep_spec_id>__*` operation this node calls) — pinned by the deterministic gate `_validate_component_dep_operations` (V4c-i, below). No `infrastructure` node is in the set: the runner harness belongs to the target the node is built for, and the host adds it to every pipeline of the node (issue #284). Do NOT author `transitive_deps` / `all_nodes`: the conductor derives that closure/topo graph host-side into `<ir_ref>/dependency_graph.json` (a pure function of `deps.yaml` + `spec_catalog.yaml`).
...
- **V4c-i (component `operations` non-empty) — deterministic Compile gate.** Every `component/`
  ...
  requires the import and the call, so a `pure` leaf cannot converge (retry budget exhausts). `problem`
...
- **V4c-ii (operations ⊆ published) — deterministic Compile gate.** Each `component/` dep's
  ...
  `source` tag of `ir_public_api` / `unresolved`) at Compile-phase start, and
...
- `python3 tools/validate_pipeline_semantics.py --stage compile --ir-ref workspace/ir/<node_key_safe>/<ir_id>/`, with `exit code 0` required. The result is recorded in `compile_static_meta.json`; on `fail` the substep fails before `Compile.verify` runs, so `verification_status=pass` is never reached on a structurally-invalid IR. (The `--stage compile` validator checks the internal-consistency / shape-grammar invariants; the spec-cross-reference invariants are the `Compile.verify` LLM responsibility — see the substep structure note.)
  ...
  - **harness render preconditions** (`_validate_harness_render_preconditions`, every non-`infrastructure` node, asked once per declared target profile whose toolchain the host renders a runner for, with that target's language and harness — the IR is target-free, so it must render for each; issue #284): the node's runner is host-rendered from this IR alone, so IR *content* the renderer cannot render is caught here instead of at the render backstop (which terminates the workflow rather than retrying). The gate delegates to `tools/host_render.ir_content_violations`, which invokes the language backend's renderer itself, so it cannot drift. Each violation is a `Compile fail` routed to `Compile.generate`:
    ...
    - no snapshot variable is named `t` / `case_id` / `step` (harness-reserved snapshot keys); every snapshot variable is a bindable identifier — it IS a module-level variable of the checks module, which the runner binds under the alias `sb_<name>` (Z6, issue #255) — so it matches `[A-Za-z][A-Za-z0-9_]*`, its `sb_` alias fits the 63-character identifier bound, no two names fold to one identifier (the bound target languages include a case-insensitive one), and none folds to a checks-ABI procedure name (`case_setup`, `case_run`, `get_time`, `checks_compute`, `metric_compute`);
    ...
    - `case.test_case_set[]` is non-empty with pairwise-distinct `case_id`s (duplicates render overlapping case labels in the host-rendered runner's case dispatch), each ≤ 64 chars — the harness `case_id_len`, which `__parse_cases` truncates a longer id to, so the runner's case label could never match and every run would stop with an error despite compiling — and each matching `[A-Za-z0-9._][A-Za-z0-9._-]*` with no `..`: a case_id is concatenated into the per-case snapshot path (`raw/state_snapshots/<case_id>.json`), so a `/` or `..` would let the run write outside its directory, and a leading `-` would be read as an option in the runner's argv;
    ...
    - every name the renderer embeds in the runner (`case_id`, snapshot variable, metric address, `test_id`, the target's hardware class) is printable ASCII — a control character has no literal form in the runner's language, and a non-ASCII character makes the rendered byte length disagree with the code-point length every render bound is measured in — and no rendered line exceeds 100 columns, so an over-long IR-sourced name is rejected rather than emitted as an unlintable runner.
  - Node-**identity** preconditions are excluded from these gates because re-authoring the IR cannot repair them: `spec_id` ≤ 55 characters (which keeps the derived `<spec_id>_runner` / `_checks` / `_model` identifiers within 63 characters, the shortest identifier bound an implemented language backend declares), and **no** `infrastructure` dependency declared in `deps.yaml` on any `spec` (issue #284: the runner harness is the attribute of the target profile a node is built for, `spec/targets/<target_id>.yaml` `harness`, and the host adds it to every pipeline of a non-`infrastructure` node; a declaration is refused as `infrastructure_dependency_declared_in_deps`, and inside a `profile` as `profile_declares_infrastructure` at the Compile start of each ADOPTING node). Both are enforced at **spec-input**, before any phase runs — for the target `spec` and for every member of a `--with-deps` closure alike. A `deps.yaml` that is **missing or whose `dependencies:` schema is malformed** is likewise rejected at spec-input on a non-`infrastructure` `spec`, since its dependency declaration cannot be read. A resume re-resolves its node through the same gate, so the remedy is to fix `deps.yaml` (or rename the `spec`) and start a new run.
...
- `minor`: the defect lies in `spec.ir.yaml`, and `last_fail_reason` names a correction that keeps it faithful to `controlled_spec.md`, `tests.md`, `deps.yaml` and `spec/registry/spec_catalog.yaml`. Every V1–V4 or V8 finding whose subject is the IR takes this value — a `case_id` `tests.md` requires and `case` lacks, a local operation lowered without the defining math `controlled_spec.md` carries, a declared `execution_mode` that contradicts the control structure `controlled_spec.md` describes, a `test_predicates` entry that mistranslates its `tests.md` prose, a `required_raw_variables` list that omits a recompute input — unless the `critical` bullet's condition holds, which wins over this universal.
...
- By keeping the IR **target-free** (issue #284), one Compile serves every target: the target — toolchain, hardware, parallel model, harness — is the host's profile, chosen per run, and the lowering onto it is the `Generate` producer's `target_lowering_plan`, so the Tune optional flow varies the lowering plan rather than the IR. Until then the IR carried both as `impl_defaults` (a fixed layer and a Tune knob layer), which made every IR a statement about one target.
