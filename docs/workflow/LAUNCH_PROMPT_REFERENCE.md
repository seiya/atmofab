...
The `generate/generate` and `generate/verify` rows apply only to a residual **agentic** generate leaf (a non-`M3c` node — the pure executor is the only executor since `M-F`, but such a node cannot be expressed as a pure producer). On an `M3c` `pure-function leaf` (`Z2`, see the Z2 section below) those substeps carry **no** `skill_name` / `skill_ref` — the leaf reads no SKILL, and the host inlines the closed context into the prompt body; `validate_bundle` is the post-generate gate the host runs on the returned `CodegenBundle`. Claude uses strict tool-free isolation; Codex uses the sandboxed structured-output approximation.
...
A pure-function leaf (`docs/design/zero_base_architecture.md` Z2, canonical `tools/pure_leaf.py`) is a host-mediated closed-context turn: the host inlines all required context, and the model returns **exactly one JSON document** (a `CodegenBundle` for `generate.generate`, a verify verdict for `generate.verify`) which the host validates and writes itself. Claude uses a `claude -p` turn with tools disabled. Codex uses `codex exec --output-schema` in a read-only sandbox: a sandboxed structured-output approximation, not strict tool-free isolation. A pure launch is confined to the migrated `(generate, generate)` and `(generate, verify)` substeps; any other `(step, substep)` with `leaf_mode=pure` is rejected at `_validate_launch_request_payload`.
