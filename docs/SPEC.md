...
- `source_meta.json` no longer records `lint_command_ref`: `static lint` is the deterministic conductor-run `Generate.gate` lint check, certified by `post_generate` against the host-authored `<pipeline_root>/lint_evidence/<source_id>.json`.
...
- `spec.ir.yaml` integrates and holds the following 4 sections (plus `public_api` on a `component` or `infrastructure` node). It names no implementation target: the toolchain, hardware, parallel backend and harness are the target profile's (`docs/IMPL_PLAN_SPEC.md`), and a Compile reply that carries an `impl_defaults` section is refused by the host (until R4-a PR-3, issue #284, the IR carried one):
...
- **Implementation discretion (B) is held outside the IR**: the target profile (`spec/targets/<target_id>.yaml`) fixes the target, and the CodegenBundle's `target_lowering_plan`, authored by `Generate`, fixes how the computation is lowered onto it (`docs/IMPL_PLAN_SPEC.md`). In the core workflow both are fixed values. Variant exploration of implementation discretion is the responsibility of the optional flow `Tune`; `Tune` treats `spec.ir.yaml` and the target profile as invariant, separately reads `tuning.spec`, and generates lowering-plan variants.
...
- By holding the physics algorithm (A) in the target-free IR and implementation discretion (B) in the target profile and the lowering plan, both physics reproducibility and Tune exploration are achieved.
...
9. Every `spec` declares its dependencies in `deps.yaml`, and direct path references (relative `import`) are forbidden. **No `spec` declares an `infrastructure` (R1 harness) dependency**, whatever its `spec_kind`: `deps.yaml` has no `infrastructure:` section. The harness is an attribute of the target profile (`spec/targets/<target_id>.yaml` `harness`), and the host adds the target's harness as a direct dependency of every non-`infrastructure` `node` built for that target (`docs/GLOSSARY.md` §1 `infrastructure spec`); an `infrastructure spec` authors its own self-test runner. Like the `spec_id` bound of requirement 4, this is checked at **spec-input**, before any phase runs, for the target `spec` and for every member of a `--with-deps` dependency closure alike; a declaration is refused there as `infrastructure_dependency_declared_in_deps` and is resolved only by editing `deps.yaml`. A `profile` reaches neither capture point — it is refused as a target and expanded away before it is visited as a closure member — so a declaration inside it is refused where the expansion happens instead, at the Compile start of every adopting `spec` (`profile_declares_infrastructure`). Until R4-a PR-3 (issue #284) the rule was the reverse: every `spec` that builds declared exactly one `infrastructure` dependency.
...
- `CI` must `pass` the existence verification of each `spec`'s `tests.md` and the `L0`-test existence verification, over every `spec_kind` but `profile` (requirement 5: a `profile` has none, and one present there is the error).
...
- The optional flow `Tune` can explore the lowering plan (B), and can evaluate performance improvement while maintaining physical passing.
