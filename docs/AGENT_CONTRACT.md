...
- Without explicit instruction, do not view or reference the content of existing/past workflow output (past `ir_id` / `pipeline_id` / `source_id` / `binary_id` / `run_id`) — do not go and `Read` other nodes' artifacts. Use as input only the repository-managed `spec` canonical source and the preceding artifacts of the relevant trial. (Exception: a **conductor-injected** `Certified exemplar` block already IN your launch prompt is permitted prior art — it is host-selected and rendered for you, NOT your own spontaneous filesystem reference; use it as a structural reference only, and never copy its physics — see the R5 exemplar rule in `skills/workflow-generate-generate/SKILL.md`.)
