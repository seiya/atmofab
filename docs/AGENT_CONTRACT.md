...
- **Every path inside your `read_manifests/<agent_run_id>.json` `allowed_read_roots` is read directly** — `Read`, `Grep`, `Glob`, or a `Bash` reader. The guard enforces the boundary on all four: `Grep` / `Glob` are validated on `path`, and **a call with no `path` searches the repository root and is blocked** — always pass an explicit `path`. `Glob`'s `pattern` is validated too, where it LANDS: `../x/*` is judged under `x/` (allowed iff `x/` is granted); an absolute or `~` pattern is blocked. `Grep`'s is a content regex and is not. A `Bash` command is validated on its extractable read targets (blocks report `"via": "bash"`).
