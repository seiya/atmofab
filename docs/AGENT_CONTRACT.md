...
- When a temporary file is needed, do not specify `/tmp` / `/dev/shm` directly; write it with the `Write` tool to the literal path of `allowed_tmp_root` (`workspace/tmp/<agent_run_id>/...`), for every extension alike. A `Write` there is auto-approved by the hook; a Bash redirect write (`cat > ... <<'EOF'`) matches no committed `permissions.allow` rule and costs an attempt on a permission refusal. The only sanctioned Bash redirect is the stderr capture appended to an already-permitted command (next item). `output_manifest_write_guard` judges only whether the write-target path is under `allowed_tmp_root` and does not reference the `$TMPDIR` env (`tools/hooks/common.py:_validate_write_access`). Bootstrap Bash such as `export TMPDIR=...`, `jq -er ...`, `printenv`, `bash -c '...'` is forbidden (the workflow stops on a session-sandbox approval request). Hard-coding `/tmp/` / `/dev/shm/` remains blocked by `output_manifest_write_guard`.
