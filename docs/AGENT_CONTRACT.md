...
- For a `run-gate` (e.g. `orchestration_read`) result, use its **stderr** as the canonical path: the gate result JSON is the last line of stderr, and stderr is returned in the command result. Read it there; do not append a redirect to capture it to a file, because the permission layer refuses a Bash redirect to a file. The runtime also leaves that JSON at `workspace/tmp/<agent_run_id>/gate_results/<gate>.json`, one file per gate name — read it there when the command result has scrolled out of your context, and check its `args_json` / `evaluated_at`: it is the last run of that gate that COMPLETED, so a refused re-run leaves the previous result in place. For the attempt you just made the command result is authoritative, not the file. On failure, `violations[]` is output to stderr in JSON form. For a summary, refer to `agent.summary.txt` / `step_result.json`. Another agent's internal artifact (files under `capabilities/` / `output_manifests/` / `read_manifests/` / `access_logs/` / `agents/<other_agent_run_id>/` / `dialogs/` that do not correspond to your own `agent_run_id`) must not be read directly. A cross-agent read is a recoverable `read_manifest_read_guard` block.
