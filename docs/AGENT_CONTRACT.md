...
- **Every path inside your `read_manifests/<agent_run_id>.json` `allowed_read_roots` is read directly** — `Read`, `Grep`, `Glob`, or a `Bash` reader. The guard enforces the boundary on all four: `Grep` / `Glob` are validated on `path`, and **a call with no `path` searches the repository root and is blocked** — always pass an explicit `path`. `Glob`'s `pattern` is validated where it lands, and the tool searches only under `path` — a `../` pattern returns nothing rather than an error, so **reach another granted root with `path=`**. `Grep`'s pattern is content, not a path. A `Bash` command is validated on its extractable read targets (blocks report `"via": "bash"`).
