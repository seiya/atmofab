...
- Your own `output_manifests/<agent_run_id>.json` and `read_manifests/<agent_run_id>.json` (under `workspace/orchestrations/<orchestration_id>/`) are readable directly even when outside `allowed_read_roots`.
- **Every path inside your `read_manifests/<agent_run_id>.json` `allowed_read_roots` is read directly** — `Read`, `Grep`, `Glob`, or a `Bash` reader. The guard enforces the boundary on all four: `Grep` / `Glob` are validated on `path`, and **a call with no `path` searches the repository root and is blocked** — always pass an explicit `path`. `Glob`'s `pattern` reads only under `path` and is gitignore-style: with a `/` it anchors at the REPO root (`docs/**/*.md`, not `sub/*.md`); without one it matches the basename at any depth. `../x/*` finds nothing — use `path=`. A pattern beginning with `/` or `~` is refused unless it points inside a granted root (`/` re-roots the search; `~` reads nothing but is refused anyway). No relative spelling is refused. A `Bash` command is validated on its extractable read targets (blocks report `"via": "bash"`).
- `run-gate orchestration_read` is an **audited re-read of an in-manifest path**, not a way around the manifest: for a path outside `allowed_read_roots` it records a `rule_source_violation` and fails the orchestration. **Such a path is unreadable by every route**, so fix a read block by re-issuing under `allowed_read_roots`, never by reaching for this gate. Forbid calling `orchestration-read` directly.
...
- An artifact write via shell redirection / `tee` / `sed -i` / an arbitrary command, and any write outside `allowed_output_paths`, remain forbidden; scratch is governed below.
...
- When a temporary file is needed, do not specify `/tmp` / `/dev/shm` directly; write it with the `Write` tool to the literal path of `allowed_tmp_root` (`workspace/tmp/<agent_run_id>/...`), for every extension. A `Write` there is auto-approved by the hook. A Bash redirect is not a write route in any position: on the Claude Code backend the permission layer refuses it as the whole command (`cat > ...`, which also matches no committed `permissions.allow` rule) and appended to a permitted command alike. `output_manifest_write_guard` judges only whether the write-target path is under `allowed_tmp_root` and does not reference the `$TMPDIR` env (`tools/hooks/common.py:validate_write_access`). Bootstrap Bash such as `export TMPDIR=...`, `jq -er ...`, `printenv`, `bash -c '...'` is forbidden (the workflow stops on a session-sandbox approval request). Hard-coding `/tmp/` / `/dev/shm/` remains blocked by `output_manifest_write_guard`.
- For a `run-gate` (e.g. `orchestration_read`) result, use its **stderr** as the canonical path: the gate result JSON is the last line of stderr, and stderr is returned in the command result. Read it there; do not append a redirect to capture it to a file, because the permission layer refuses a Bash redirect to a file. The runtime also leaves that JSON at `workspace/tmp/<agent_run_id>/gate_results/<gate>.json`, one file per gate name — read it there when the command result has scrolled out of your context. It holds the last run of that gate that COMPLETED — a run that reached the runtime and was refused removes it first, but one refused before that (a malformed command line, a Bash command the permission layer refused) never ran — so check its `args_json` / `exit_code` / `evaluated_at` against the run you mean, and note it says nothing about whether the files it names have changed since. For the attempt you just made the command result is authoritative, not the file. On failure, `violations[]` is output to stderr in JSON form. For a summary, refer to `agent.summary.txt` / `step_result.json`. Another agent's internal artifact (files under `capabilities/` / `output_manifests/` / `read_manifests/` / `access_logs/` / `agents/<other_agent_run_id>/` / `dialogs/` that do not correspond to your own `agent_run_id`) must not be read directly. A cross-agent read is a recoverable `read_manifest_read_guard` block.
- **Sandbox facts (do not re-derive these — they are fixed):** Your **launch prompt's Gate Runbook already lists the exact, fully-resolved gate command(s) for your `(step, substep)`** with every path/id/scoping substituted; run them verbatim rather than reconstructing the arguments. (1) Your **cwd is the repo root** (`.`) and persists across `Bash` calls — **do not `cd`**; a `cd` breaks the workspace-relative paths every gate and write guard expects (use repo-root-relative paths in every command). (2) `workspace/tmp/<agent_run_id>/` is **pre-created and writable** before you launch — write there directly with the `Write` tool; never test for or `mkdir` it. (3) `access_logs/<agent_run_id>.jsonl` is **runtime-managed audit**: the read hook appends a line per read decision, and a `run-gate --gate orchestration_read` writes one too. You never read or write that file — do not treat those appends as something you must handle.
...
- Use the `Read` tool to confirm the content of a JSON file. `python3 -c "import json; ..."` is blocked by `forbid_python_inline_write`. Only when Python processing is truly necessary, write a script to `workspace/tmp/<agent_run_id>/x.py` with the `Write` tool and run it with `python3 workspace/tmp/<agent_run_id>/x.py` (`<agent_run_id>` is literally substituted).
...
- Generate `diagnostics.json` / `perf.json` only as the real execution result of the target `runner`. Forbid hand-writing, fixed-value embedding, and external post-editing. `verdict.json` / `aggregate_verdict.json` are **not leaf-authored** — the conductor derives them deterministically (`verdict.json` at `execute` from `io_contract.test_predicates` + `diagnostics.json` — R2; `aggregate_verdict.json` in `post_judge` — G6) from the same-`run_id` execution artifacts.
- On a phase failure, do not artificially generate an artifact to satisfy a downstream phase's start condition.
- Without explicit instruction, do not view or reference the content of existing/past workflow output (past `ir_id` / `pipeline_id` / `source_id` / `binary_id` / `run_id`) — do not go and `Read` other nodes' artifacts. Use as input only the repository-managed `spec` canonical source and the ids YOUR launch request names — the host may hand you an `ir_ref` an earlier run produced and this one adopted; that is its decision to make, and not a reason to refuse or to look for others. (Exception: a **conductor-injected** `Certified exemplar` block already IN your launch prompt is permitted prior art — it is host-selected and rendered for you, NOT your own spontaneous filesystem reference; use it as a structural reference only, and never copy its physics — see the R5 exemplar rule in `skills/workflow-generate-generate/SKILL.md`.)
