...
- **Every path inside your `read_manifests/<agent_run_id>.json` `allowed_read_roots` is read directly** — `Read`, `Grep`, `Glob`, or a `Bash` reader. The guard enforces the boundary on all four: `Grep` / `Glob` are validated on `path`, and **a call with no `path` searches the repository root and is blocked** — always pass an explicit `path`. `Glob`'s `pattern` matches the REPO-relative path and reads only under `path`: for `path="docs"` write `pattern="docs/**/*.md"` (`sub/*.md` and `../x/*` find nothing); reach another root with `path=`. A `Bash` command is validated on its extractable read targets (blocks report `"via": "bash"`).
