...
- A `Generate.gate` lint verdict is a function of the source and the declared set, not of the
  host's build — for every source that obeys the contract the leaf-read documents state.
- **One measured exception, stated rather than implied.** `S001` compares the line length with
  `>=` on 0.8.x and with `>` on 0.9.x, so a line of EXACTLY 100 columns fails on 0.8.0 and passes
...
- **How loud the allow-comment closure is depends on what the directive names.** A code OUTSIDE
  ...
  violates it earns nothing of its own — the suppressed finding simply fires. The leaf-facing rule
  is therefore "write none", not "you will be told". An earlier version of this document said the
  closure was always `FORT005`, which is the case a leaf would least often be in.
...
Verdicts. The CONTROL is reproducible anywhere; the INCIDENT subject is not, and saying so is
part of the record: it is a run artifact under `workspace/`, which this repository does not
track, so only a machine that has that run can re-take those two rows. The incident source is
the three files of
...
- **The leaf-facing documents** state individual rules of this set to the agent that must satisfy
  them (`docs/backends/language/fortran/GENERATE_RULES.md` §1, the Fortran prompt fragments
  `tools/prompt_templates/backends/language/fortran/generate_generate.txt` the host composes
  into `tools/prompt_templates/pure_generate_generate.txt`, and
  `docs/backends/language/fortran/CHECKS_ABI.md` §5). Each names a subset and cites this set. A code
