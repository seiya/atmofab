...
| `target_unresolved` | a `generate` / `build` / `validate` phase was asked without a target: no `--target`, and the orchestration records no `invocation.target` (one launched before issue #284) or names a profile that no longer loads | pass `--target`, or start a fresh run — a pipeline built for no recorded target is not in the per-target store |
