...
- **Scope of application.** These premises decide what is DEFENDED, not what already exists. An enforcement layer written under a wider assumption is not deleted by this section; deleting a defense is a separate judgment with its own evidence requirement (`.claude/skills/metdsl-enforcement-change` rule 1-b).
