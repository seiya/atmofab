...
- The canonical entrypoint for starting the workflow is the user running `python3 tools/run_workflow.py <spec_ref> <until_phase> [--llm <codex|claude>]`; a Codex run also requires `--agent-model <explicit-model-slug>` (add `--with-deps` to run the dependency closure, `--resume` to recover a failed run; see `docs/RUNBOOK.md`).
