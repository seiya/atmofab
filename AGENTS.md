...
- **Scope of application.** These premises decide what is DEFENDED, not what already exists. An enforcement layer written under a wider assumption is not deleted by this section; deleting a defense is a separate judgment with its own evidence requirement (`.claude/skills/metforge-enforcement-change` rule 1-b).
...
- When the workflow runs, the canonical source for `METFORGE_WORKFLOW_MODE=1` and `METFORGE_ORCHESTRATION_ID=<orchestration_id>` is the values set by the `tools/run_workflow.py` that the user started.
