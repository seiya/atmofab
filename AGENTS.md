...
- Under the workflow, `compile_project` / `run_program` / `run_quality_checks` / `run_linter` / `run_syntax_check` require `orchestration_id`, `agent_run_id`, and `capability_token`; a call that omits them is refused, not exempted from the capability gate. There, `env` accepts only the key names `OBJDIR` / `BINDIR` / `RUNDIR` / `BIN` / `SPEC` / `CASES`, `compile_project`'s `extra_args` accepts only assignments to those make variables, and `run_syntax_check`'s `sources` accepts only plain source file names; everything else is refused. Canonical: `mcp_servers/README.md`.
