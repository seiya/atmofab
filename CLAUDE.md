...
>
> The one rule that used to arrive this way and has no second home for a leaf — `AGENTS.md` §Backend boundary rules — is not actionable by a leaf, because a leaf cannot write a `neutral core` file. A leaf has NO repository write authority: the host writes every artifact from the document the leaf returns, and the leaf's own sandbox binds the checkout read-only. (`_write_roots_for_launch` was the sole producer of write authority and is deleted with the capability document in PR-2 of that issue; the roots it returned were under `workspace/orchestrations/<id>/`, `<ir_ref>/`, `<pipeline_ref>/` or — for `promote` alone — `releases/<spec_kind>/…` plus `spec/registry/spec_catalog.yaml`, none of them `neutral core`.) So the rule is out of a leaf's reach twice over.
