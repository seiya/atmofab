...
plus `git checkout -- .`, and `metforge-review-loop` forbids `git checkout -- <path>` by name for
...
Use `.claude/skills/metforge-review-loop/scripts/mutation_check.py`. The procedure is owned by
`metforge-review-loop`'s "Before you hand it over (round 0)".
