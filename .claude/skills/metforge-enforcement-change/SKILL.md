---
name: metforge-enforcement-change
...
checking — is owned by `metforge-review-loop`, so start that one too once you reach the review
stage.
...
enumeration by mutation** (round 0 in `metforge-review-loop`). A missing element shows up in no
other test.
...
- **Surface 5 — is caller-controlled data mixed into the classification channel the verdict
  ...
  of `metforge-review-loop`'s sign "the pin was broken in a different shape every time — move the
...
The procedure and the script are **owned by `metforge-review-loop`**
