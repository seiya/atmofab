...
validator, hook, or capability), **start `metforge-enforcement-change` as well** — it owns the
...
python3 .claude/skills/metforge-review-loop/scripts/mutation_check.py \
...
3. **Run the verification set** and record the measurements. The commands are in
   `.claude/skills/metforge-enforcement-change/references/verification.md` (suite baseline, ruff
...
"the implementation is right but the test is weak" as real. (`metforge-enforcement-change` judgment
...
- **Buying the leaf nothing toward done** — the operator's credentials, a read outside the
  ...
  nothing, stop. That permission is narrow and is not rule 1's — `metforge-enforcement-change`
...
- **You have rewritten the same string three times** → the problem is not the rule but the prose
  ...
  (`.claude/skills/metforge-enforcement-change/references/verification.md`). **Rewriting one
  statement repeatedly is a SWEEP problem, which this row owns; several sites that each state the
  rule is a COUPLING problem, which `metforge-enforcement-change` rule 3-a owns and states the
...
- **A reviewer said "it is environment-dependent"** → do not close it with a mock on the test side.
  Ask first what happens in production on that environment (`metforge-enforcement-change` rule 2
  owns this)
...
- **Your assertion searches a TOOL'S OUTPUT for a code, a name, or a marker** → check what else
  ...
  This is `metforge-enforcement-change`'s surface 5 — caller-controlled data mixed into a
