...
rather than because of your change.** In met-forge the default worktree location has a
...
the tests that could plausibly see the change. Measured on a 4-hunk met-forge range with
...
met-forge's standing instance is the two path-depth-coupled `ForbidBackendCredentialReadTests`
...
#: exactly the prompt-template and contract text met-forge pins. Two review rounds broke a
...
def _suite_did_not_run(output: str) -> bool:
    ...
    test noticed the change when no test ran at all. A witness census on met-forge PR #68 hit exactly
...
def _docstring_only(repo: Path, head: str, path: str, patch: str) -> bool:
    ...
    met-forge pins prompt-template and contract text. But when nothing does, such a hunk is a
...
def main() -> int:
    ...
    # TMPDIR set below and every job shares one temp root. That is the configuration met-forge
    ...
    # characters longer, and met-forge has at least one test whose budget the checkout
    ...
    # Named as short as `mkdtemp` allows: a met-forge test asserts a budget on a message
    ...
    # `--test-cmd` in met-forge: two `DevShmWriteBlockTests` rows reason about a write guard
    ...
    if survivors:
        ...
        if prose:
            print("\nA prose-only hunk — comments, docstrings — changes no behaviour, so it "
                  ...
                  "does) and it is labelled so it does not read as a finding: on met-forge PR #67, "
    ...
    print("NOT the same as 'the tests are adequate'. Reverting a hunk cannot detect a test\n"
          ...
          "unobserved. In met-forge L128 the whole scope analysis could be replaced by a\n"
