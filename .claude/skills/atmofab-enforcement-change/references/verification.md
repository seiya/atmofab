...
## The backend-boundary token ratchet

`TokenRatchetTests` in `tools/tests/test_backend_boundary.py` counts technology tokens per file and
fails on growth or a stale entry; `python3 -m tools.tests.test_backend_boundary --check-baseline`
runs the same comparison on its own and exits 1 naming the entries.
...
```bash
python3 -m tools.tests.test_backend_boundary --check-baseline      # the sampled comparison alone; exit 1 names the entries
...
**A ratchet failure is a false KILL in a mutation sweep.** Deleting an identifier to test
something else moves the count; the ratchet fails and the mutant reads as killed for a reason that
has nothing to do with behaviour, and on TODO:269 that hid an exit-code mapping with no behavioural
witness at all. When a mutant dies, read WHICH row died — and deselect `TokenRatchetTests` from a
sweep's `--test-cmd` when the mutants delete identifiers.
