...
| the hook's `repo_root` | `_resolve_repo_root` (which tree the POLICY is evaluated against) | `_append_hook_audit` (which tree the RECORD lands in) | Unified into `_repo_root_from_sources` (issue #102). They drifted TWICE in one branch: first the audit preferred the payload while the policy preferred the environment, then — after the ORDER was unified — the audit still read the nested (codex) key that the policy never looked at. Both times the decision and its record named different trees, which is a false record rather than a missing one, and both times production hid it because the committed wrappers set both sources to `$ROOT`. **Unifying the ORDER is not unifying the SOURCES**; spelling the list twice is what let it drift twice. The callers now differ only in what "no source" means, and each says why |
| `ATMOFAB_WORKFLOW_MODE` | `tools/hooks/cli.py` (`== "1"`, one reader) | server (anything but empty and `0` is workflow) | The server is the widest = the fail-closed side. The allowlist spelling `{1,true,yes}` was the hook's SECOND reader until issue #102 deleted it with `_env_flag_true`; a branch that widens it again re-creates the pair |
| the leaf's configuration layer | preflight `_read_repo_mcp_tool_permissions` and the tests that read the hook wiring | the real files `_prepare_claude_workflow_home` pins and copies | Unified into `leaf_config/claude/settings.json` (issue #63). **A sync test compares the GRANTS against the dev layer `.claude/settings.json`. It no longer matches the hooks: issue #102 separated the layers to the entrypoint, and what replaced the subset match is `HookLayerSeparationTests`, asserting the two share no hook command and that no dev command names `tools.hooks.cli`.** During the migration only the Bash allowlist pin kept reading the dev layer, and **deleting all 16 entries on the leaf side left everything green** |
| where the leaf's transcript lives | `_claude_session_resumable` (whether a warm resume is possible) | `orchestration_diagnostics._locate_leaf_transcript` / `_claude_projects_dir` (after-the-fact audit) | `tools/hooks/common.py::claude_leaf_projects_roots` was added for the audit side (issue #63); `_claude_session_resumable` deliberately does NOT call it and builds the one home a launch uses. **The answer differs by purpose**: the audit is right to look at both the private home and the operator home, while resume must look **only at the one that launch uses**. The version that looked at both called a pre-move session resumable and **threw `--resume` at a home that does not exist** |
| where the isolated home lives | the side bwrap binds (`claude_isolation_profile_kwargs`) | the side the Bash read guard forbids (`workflow_private_backend_homes`) | Issue #63 **moved one side only** and opened a hole: binding the operator's credentials into the private home made **the same secret appear at a path the guard does not know** (`~/.claude/...` blocked, `<home>/...` allowed). **CLOSED before this table came into the repository** — the guard now consults `workflow_private_backend_homes`, and `_backend_runtime_bind_paths` states the scope of its own claim for `backend_rw_override` callers. **The home MOVED AGAIN in issue #64**, to `~/.atmofab/homes/<oid>/<backend>`, and the first account of that move was too kind to itself: "the new location is under a root the guard already refuses, so enforcement moved for free". Not free. `ATMOFAB_WORKFLOW_HOMES_ROOT` relocates the tree, and with it pointed outside `~/.atmofab` a leaf could read a SIBLING orchestration's transcript (measured, by Codex) while three documents asserted the closure with no mention of the condition — the pair had come apart again along an axis nobody had looked at. It took a code change (the homes ROOT became a protected entry, resolved by the same function the writer uses) to make the claim true. **The lesson the row now carries: "one side moved under something the other side already covers" is a claim about a CONFIGURATION, and it holds only for the configurations you enumerated.** Kept as a row because the SHAPE recurs: one side of a bind/forbid pair moved without the other |
| where the operator's dismiss-violation token lives | the WRITER `init_orchestration` and the READER `dismiss_violation` (`Path.home() / ".atmofab" / "operator_tokens"`, spelled inline in each) | the Bash read guard's `operator_secret_root()` — the root ABOVE the store, which covered it by placement | **RESOLVED = PR #140 (issues #132 / #133)**, and the row stays because the resolution is the shape to copy. Four independent spellings of `~/.atmofab` existed, agreeing by coincidence; the #127 rename moved the guard's and left the data behind for one migration step, which is the disagreement this table asks for made real. `tools/hooks/common.py::operator_tokens_root` is now the only one and the writers IMPORT it, so the tree that gets created is the tree that gets guarded — the arrangement `backend_credential_home_paths` already used. **Two traps the fix hit, both worth more than the pair itself.** (1) The store is a LONGER path than `~/.atmofab` and the roots sort longest-first, so adding the entry alone silently moved every token read into `forbid_backend_credential_direct_read`; the policy branch has to treat the two as one class, and the default block message has to stay byte-identical or every document naming it becomes false. (2) The Bash guard is not the only reader of "may a leaf reach this": `_write_read_access_manifest` grants `docs/` and `spec/` unconditionally and **never consults `protected_host_read_roots`**, so the new relocator made a store at `<repo>/spec/tokens` readable by every leaf — a `leaf shortcut` the new lever created, found by Codex. Adding a relocator to a protected thing adds a row to THIS table on the manifest side too. Held by `test_the_three_writers_and_the_guard_resolve_one_root` (drives all four sites under one patched `_home_dir`) and by a census pinning the `".atmofab"` literal to one spelling |
