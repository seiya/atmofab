...
| where the isolated home lives | the side bwrap binds (`claude_isolation_profile_kwargs`) | the side the Bash read guard forbids (`workflow_private_backend_homes`) | Issue #63 **moved one side only** and opened a hole: binding the operator's credentials into the private home made **the same secret appear at a path the guard does not know** (`~/.claude/...` blocked, `<home>/...` allowed). **CLOSED before this table came into the repository** — the guard now consults `workflow_private_backend_homes`, and `_backend_runtime_bind_paths` states the scope of its own claim for `backend_rw_override` callers. **The home MOVED AGAIN in issue #64**, to `~/.met-dsl/homes/<oid>/<backend>`, and this time the pair held for a reason worth recording: the new location is under a root the guard already refuses unconditionally, so the enforcement side moved for free and only the ATTRIBUTION depended on the resolver. Kept as a row because the SHAPE recurs: one side of a bind/forbid pair moved without the other |
