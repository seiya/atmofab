...
`pytest.ini` registers the `slow` marker, sets `-p no:randomly`, and points `testpaths` at
`tools/tests` so a bare `pytest` at the repository root runs the suite. The marker is
...
instead of about the code. Measured at `165c26f` (to re-take at `165c26f`, which predates the rename, spell the names `METDSL_*`; `docs/GLOSSARY.md` §13): exporting every such name the tree reads took a
clean run of 5280 passed to 181 failed, none of the failures belonging to any change. Every run says
