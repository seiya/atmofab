...
- `level`: `L3`
- `objective`: confirm, through the pair of 2-8, that the time integration and the forcing time are those of the Controlled Spec §5: the pair excludes the second- and third-order Runge–Kutta methods, the three-eighths-rule fourth-order method, a forcing time other than the stage time, a forcing built from the stage state, and the `cfl_target` values listed in 2-8.
- pair cases:
