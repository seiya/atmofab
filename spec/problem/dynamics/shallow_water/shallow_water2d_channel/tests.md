...
### 2-1. Basic constants
The constants are those of the Controlled Spec §6, in SI units: `g=9.80616`, `Omega=7.292e-5`, `a=6.37122e6`, `gh0=2.94e4` (`h0 = gh0/g = 2998.1155...`), `u0 = 2*pi*a/(12*86400) = 38.6107...`, `theta0 = pi/4`, so `f0 = 2*Omega*sin(theta0) = 1.03126...e-4` and `beta = 2*Omega*cos(theta0)/a = 1.61864...e-11`. The ellipses mark derived values: the case inputs carry the constants `g`, `Omega`, `a`, `gh0` and `theta0` (the double-precision value of `pi/4`) as numbers, and `h0`, `u0`, `f0`, `beta` are evaluated from them by the expressions above wherever they are used — by the runner and by a judgment that evaluates a reference field alike; no rounded literal of a derived value is an input.
...
Both profiles are steady solutions of the continuous equations, so the reference at $t_{end}$ is the initial state: for `tc2_zonal_uniform` the closed form of 2-3 evaluated at the cell centres, $h_{ref}(y)=h(y)$; for `tc3_compact_jet` the discrete initial state itself, $h_{ref}=h(t=0)$ (its $h$ has no closed form). The theoretical-agreement judgment targets `h`; a judgment for `hu` / `hv` against the reference is not required in this suite, and the wall-normal velocity is judged by its own metric (2-5).
...
- `errors.initial_h.linf_rel` is `N/A` for anything other than `flow_profile=tc2_zonal_uniform` (the `tc3_compact_jet` initial `h` is defined by the quadrature and has no independent closed form).
- `errors.initial_u.linf_rel` is `N/A` for anything other than `flow_profile=tc3_compact_jet` (the `tc2_zonal_uniform` initial `u` is the constant `u0`, judged through `errors.initial_h.linf_rel`).
...
$$
\mathrm{initial\_h\_linf\_rel}=\frac{\max_{i,j}|h_{i,j}(0)-h_{ref}(y_j)|}{\max_{j}|h_{ref}(y_j)|}
\quad(\texttt{tc2\_zonal\_uniform}),\qquad
\mathrm{initial\_u\_linf\_rel}=\frac{\max_{i,j}|u_{i,j}(0)-u(y_j)|}{u_0}
\quad(\texttt{tc3\_compact\_jet})
$$
with $u_{i,j}=(hu)_{i,j}/h_{i,j}$ and $u(y)$ the closed form of 2-3. They are emitted as `errors.initial_h.linf_rel` and `errors.initial_u.linf_rel`.
...
- judgment conditions:
  - The initial-state judgment is applied. The evaluation expression is `errors.initial_h.linf_rel`, and the threshold is $\le 1.0e{-9}$.
  ...
  - The wall-normal-velocity judgment is not applied. The non-application basis is "because the test judges the initial state, in which $v=0$ by construction".

### 6-2. `l0_tc3_initial_jet_matches_analytic`
- `level`: `L0`
- `objective`: confirm that the discrete initial velocity of `tc3_compact_jet` is the closed form of 2-3 at the cell centres.
- target cases:
  - `chan_tc3_ref_n064_dts100`
- `expected_outcome`: `pass`
- judgment conditions:
  - The initial-state judgment is applied. The evaluation expression is `errors.initial_u.linf_rel`, and the threshold is $\le 1.0e{-9}$.
  - The `CFL` judgment is applied. The evaluation expression is `cfl.max`, and the threshold is $\le 1.0$.
