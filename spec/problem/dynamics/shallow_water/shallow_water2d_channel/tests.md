...
The forcing formula is verified by the forcing `component`'s own `L0` suite, against a central-difference residual of the prescribed flow. This suite judges the forced run as a whole against the prescribed flow at $t_{end}$: the calibration of 5-5 puts a run without the forcing at `2.886e-3` / `2.994e-3` (`nx=64` / `128`) and a run with the forcing frozen at $t=0$ at `2.880e-3` / `3.037e-3`, 3.0 to 20.9 times the upper bound of `errors.mms_h.l2_rel_tend` at those resolutions, and a state left at its initial value at `6.969e-3`. A run that evaluates the forcing at the time of the step's start instead of the stage time differs from the reference implementation by `0.3%` / `1.6%` in `errors.mms_h.l2_rel_tend` (`6.394e-4` / `9.817e-5`), inside the band; that substitution is detected by the time-refinement pair of 2-8, not by the band. Four substitutions stay inside every band of `errors.steady_h.l2_rel_tend` and `errors.mms_h.l2_rel_tend`, measured on the reference implementation: a forcing evaluated from the stage state instead of the prescribed flow (`0.10%` to `0.42%` from the reference), `dubar` taken as a difference of the `ubar` row array instead of its closed form (within `0.02%`), the `hbar` row taken from the stage state (`0.09%` at `nx=32`), and a reconstruction of the primitive variables $(h,u,v)$ instead of the conserved ones (`0.8%` to `11%`). The time-refinement pair of 2-8 excludes all four (`2.352e-8`, `2.346e-8`, `2.188e-8`, `1.557e-8`). The `ubar` row taken from the stage state lies inside the pair band (`1.801e-8`) and the `nx=32` band (`2.1%` from the reference), and is excluded by the upper bound of `errors.mms_h.l2_rel_tend` at `nx=64` (`9.86e-4`) and `nx=128` (`8.40e-4`).

### 2-8. Time-refinement pair
The error against the reference at $t_{end}$ is dominated by the spatial discretization: replacing `RK4` by a second- or third-order Runge–Kutta method, or changing `cfl_target` between `0.2` and `0.6`, changes `errors.steady_h.l2_rel_tend` and `errors.mms_h.l2_rel_tend` by less than `0.05%` at `nx=32` and `64`. The time integration is judged by a pair of `tc4_translating_low` runs at `nx=32` that differ only in `dt_scale` (`1.0` and `0.5`): the relative difference of their final depths is `errors.time_pair.h_l2_rel` (5-4). Its reference value is `1.818e-8`; the pair at `dt_scale` `0.5` and `0.25` gives `3.949e-9`, a ratio of `4.6`, so the limited scheme is not in the asymptotic regime of `RK4` and the value is not the time error of a fourth-order method. The value is a property of the fixed discretization and the `n_step` of §3: perturbing the state by `1e-15` relative at every step moves it by `4e-8` of itself. A two-stage second-order method gives `2.0e-6`, `SSPRK3` `8.7e-8`, the three-eighths-rule fourth-order method `1.03e-8`, the forcing at the step's start time or end time for all four stages `3.4e-6` and at the step's midpoint for all four stages `3.05e-8`; `cfl_target` `0.3`, `0.41`, `0.42`, `0.47`, `0.48` and `0.6` give `7.2e-9`, `1.45e-8`, `1.44e-8`, `2.50e-8`, `2.45e-8` and `3.6e-8`; each lies outside the band of 5-5. The pair does not fix the stage times at which the forcing is read, nor the tableau of a four-stage fourth-order method: the classic `RK4` tableau with the forcing read at $t+\{0,\tfrac13,\tfrac23,1\}\,dt$ gives `1.869e-8`, and the four-stage fourth-order method with $c_2=0.45$, $c_3=0.55$ gives `1.703e-8`, both inside the band; the tableau and the stage times are those of the time-update `component`, which the Controlled Spec §5 calls and which that `component`'s own suite pins. The value does not change monotonically with `cfl_target` (`0.43` gives `1.78e-8`, inside the band), so the pair does not fix `cfl_target` to `0.45`; `run.n_step` does.
...
- `level`: `L3`
- `objective`: exclude, through the pair of 2-8, the substitutions of the Controlled Spec §5 that 2-8 lists: the second- and third-order Runge–Kutta methods, the three-eighths-rule fourth-order method, the forcing at the step's start, end or midpoint time for all four stages, a forcing evaluated from the stage state instead of the prescribed flow (2-7), and the `cfl_target` values listed in 2-8. The substitutions 2-8 states the pair does not exclude are not judged here.
- pair cases:
  - `reference` is `chan_tc4_ref_n032_dts100`
  - `halved` is `chan_tc4_ref_n032_dts100_half`
- `expected_outcome`: `pass`
- judgment conditions:
  - The time-refinement judgment is applied. The evaluation expression is `errors.time_pair.h_l2_rel` (carried by the halved case), and the band is $[1.60e{-8},\ 2.05e{-8}]$ as two judged quantities.
