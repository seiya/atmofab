...
One time step is `dynamics_shallow_water_time_update_2d_rk4__advance(3, nx, ny, U_n, rhs, t, dt, U_np1, guard_pass)`, with `t` the time at the start of the step, after which $t \leftarrow t+dt$. The four stage evaluations, their states and times are the time-update `component`'s (its §3); this `problem` does not evaluate a stage itself.
