# Tests: CUDA C++/GPU runner harness (L0 plumbing self-test)
...
- `test_id`: `l0_numeric_roundtrip_pass`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__emit_real`
  - `expected_outcome`: `pass`
  - `required_raw_variables`: `x_in` (rank-1, `[3]`), `x_out` (rank-1, `[3]`), `max_abs_deviation` (scalar)
  - `judgment`: the sentinel reals `x_in` (the three values §2 declares) emitted via `harness_cpp_gpu__emit_real` and re-parsed into `x_out` reproduce the inputs within an absolute tolerance of `1e-12` (`max_abs_deviation = max|x_out - x_in| <= 1e-12`), and `checks.numeric_roundtrip.status == pass`.
...
- `test_id`: `l0_boolean_literal_pass`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__emit_bool`
  - `expected_outcome`: `pass`
...
- `test_id`: `l0_array_emit_pass`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__emit_array_r2` (representative; the case exercises `__emit_array_r1..r4`)
  - `expected_outcome`: `pass`
  - `required_raw_variables`: `a1` (rank-1, `[2]`), `a2` (rank-2, `[2,2]`), `a3` (rank-3, `[2,2,2]`), `a4` (rank-4, `[2,2,2,2]`), `max_abs_deviation` (scalar)
  - `judgment`: the rank-1..4 real arrays `a1..a4` emitted via `harness_cpp_gpu__emit_array_r1..r4` produce well-formed nested JSON arrays whose parsed shape and element values match the inputs within `1e-12` (`max_abs_deviation <= 1e-12`), and `checks.array_emit.status == pass`.
...
- `test_id`: `l0_case_fanout_pass`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__write_snapshot`
  - `expected_outcome`: `pass`
...
- `test_id`: `l0_missing_cases_xfail`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__parse_cases`
...
- `test_id`: `l0_multi_case_evidence_pass`
  - `level`: `L0`
  - `operation_id`: `harness_cpp_gpu__write_metrics_basis`
  - `expected_outcome`: `pass`
