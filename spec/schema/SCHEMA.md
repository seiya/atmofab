...
- Also reference it from the surface that reaches the leaf it binds: the phase contract the host inlines into that leaf's prompt (`docs/workflow/phases/phase_01_compile.md` for the IR), or the leaf's own launch template under `tools/prompt_templates/`. (It said "a SKILL document such as `skills/workflow-compile-generate/SKILL.md`" until Z4, issue #171, which deleted the phase SKILLs with the leaf that read them.)
...
- `ir/impl_defaults.schema.json` — the canonical key names of the parallelization family inside `impl_defaults`'s knob layer (`abstract.parallelization` / `parallel_scope` / `parallel_granularity`, `backend_overrides.openmp.num_threads` / `schedule` / `chunk_size` / `collapse` / `nested`). **Declarative copy**: the validator holds the same names as constants, and `test_schema_agrees_with_the_validator_constants` pins the two copies together. The alias→canonical table is the load-bearing part and lives only in the validator, because draft-07 cannot express "this misspelling means that key"; the schema records the aliases as `x-forbidden-examples`. The knob layer stays `additionalProperties: true` — only the parallelization family is pinned, so Tune's exploration space is unaffected. Canonical validator: `tools/validate_pipeline_semantics.py:_validate_impl_defaults_knobs`. Canonical document: `docs/workflow/phases/phase_01_compile.md`.
- `targets/target_profile.schema.json` — the whole-document shape of a target profile, `spec/targets/<target_id>.yaml` (issue #284): one value per target axis and the harness the target runs over, authored by the operator and read at launch by `tools/run_workflow.py --target`. Not a phase artifact, so it sits under `targets/` rather than a phase directory. **Declarative copy**: the loader holds the same key sets as constants, and `tools/tests/test_target_profile.py` pins the two copies together. Canonical validator: `tools/target_profile.py:load_target_profile`.
