...
- `test_id`: `l0_invalid_dry_state_xfail`
  ...
  - `judgment`: with `U_L = [0, 0, 0]`, the `operation` returns `guard_ok = 0` (`checks.input_guard` is `fail`, which makes the case's `verdict.overall` `fail`); the captured `guard_ok` is `0`, not the setup value `-1`. A model whose guard on `U_L` is `h<0`, and a checks module that does not call the `operation` on this case and leaves the flag at its setup value, both fail this test.
