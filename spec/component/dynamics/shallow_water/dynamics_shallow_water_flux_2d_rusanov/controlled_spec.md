# Controlled Spec: 2D shallow water Rusanov flux from supplied interface states (component spec)
...
This `component` is responsible only for the interface-flux computation of the shallow water equation: it returns the Rusanov flux across an interface from the two states the caller supplies on its two sides. The reconstruction that produced those states is the caller's — a first-order `p0` reconstruction passes the two adjacent cell values, a piecewise-linear `p1` reconstruction passes the limited interface states of a reconstruction `component` — and the order of that reconstruction is not a property of this `component`. It does not reconstruct, does not apply a boundary condition, and does not integrate in time.

## 2. input/output contract
The input variables are the left/right states `U_L`, `U_R` across an `x`-interface and the bottom/top states `U_B`, `U_T` across a `y`-interface, each the conserved-variable vector `U=[h, hu, hv]^T`, together with the gravitational acceleration `g`. The output variables are the numerical flux `F*` across the `x`-interface and `G*` across the `y`-interface, each a length-3 vector ordered as `[h, hu, hv]` and aligned with `U`, together with the Rusanov wave speeds `a_x`, `a_y` and the input guard `guard_ok` (§4).

**Published signature.** §5.1 pins the operation's full argument list, and this paragraph describes it: a reader must be able to check the two against each other. The published arguments are `U_L`, `U_R`, `U_B`, `U_T` (3 values each) and `g` as inputs; `F_star`, `G_star` (3 values each), `a_x`, `a_y` and `guard_ok` as outputs — the published names of `F*` and `G*` being `F_star` and `G_star`.

Array placement: all inputs and outputs are interface-located values. The caller supplies the left/right and bottom/top states at each interface, whatever reconstruction produced them, and receives the flux at the same interface. The operation is pointwise per interface; vectorized application over a 2D grid is the caller's responsibility.
...
The published `operation` is `dynamics_shallow_water_flux_2d_rusanov__compute_flux`. The Rusanov flux is defined by
$$
...
$$
where the physical fluxes of the state $U=[h,\ hu,\ hv]^T$ are
$$
F(U)=\begin{bmatrix}hu\\ \dfrac{(hu)^2}{h}+\dfrac{g h^2}{2}\\ \dfrac{(hu)(hv)}{h}\end{bmatrix},\qquad
G(U)=\begin{bmatrix}hv\\ \dfrac{(hu)(hv)}{h}\\ \dfrac{(hv)^2}{h}+\dfrac{g h^2}{2}\end{bmatrix}
$$
and the wave speed is
$$
a_x=\max(|u_L|+c_L,|u_R|+c_R),\quad a_y=\max(|v_B|+c_B,|v_T|+c_T),\quad u=\frac{hu}{h},\ v=\frac{hv}{h},\ c=\sqrt{gh}
$$
...
procedures:
- kind: subroutine
  name: dynamics_shallow_water_flux_2d_rusanov__compute_flux
  args:
  - name: U_L
    rank: 1
    intent: in
...
## 6. Prohibitions
Forbid reconstructing, limiting, or otherwise modifying the supplied interface states before the flux is evaluated, and forbid evaluating the flux at a state other than the two supplied. Forbid replacing the Rusanov wave speed of §3 by another estimate.
