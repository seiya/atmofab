...
- The linear case uses `U_in(i,j) = 1 + 0.3*i + 0.2*j`.
...
- `test_id`: `l0_linear_field_exact_pass`
  ...
  - `judgment`: with the linear field, every interface state equals the value of the linear field at the interface: `U_L(m,j) = U_R(m,j) = 1.85 + 0.3*m + 0.2*j` for `m=1..7`, `j=1..4` (the field at the padded point `(m+1.5, j+2)`), and `U_B(i,n) = U_T(i,n) = 1.9 + 0.3*i + 0.2*n` for `i=1..6`, `n=1..5` (the field at `(i+2, n+1.5)`), each within an absolute tolerance of `1e-12` (component-wise max deviation `<= 1e-12`), and `grid_valid = 1` (`checks.linear_exact`). The slope of every cell is `0.3` in `x` and `0.2` in `y`, the central difference, which the `MC` minimum returns because the two doubled one-sided differences are `0.6` and `0.4`. A model with a zero slope (`p0`) gives `U_L - U_R = -0.3` at every `x` interface; a model with the slope divided by a grid spacing, or with a slope of the neighbouring cell, gives a non-zero `U_L - U_R` too.
