...
- No state variable is derived by the checks module after the run: a deviation, a maximum or a norm over these variables is a `checks.<id>` status, never a state variable. `S_fd` is derived before the run, from the §2 closed forms and the §2 inputs alone, and no element of it is read from an output of the `operation`. Every judgment of §6 is a statement about these variables and the §2 inputs.
...
- `test_id`: `l0_translation_covariance_pass`
  ...
  - `operation_id`: `dynamics_shallow_water_source_2d_tc4_forcing__apply`
  - `expected_outcome`: `pass`
  - `judgment`: the forcing at `t=106066.017177982` is the forcing at `t=0.0` shifted three cells east: the maximum over the components and cells of `|S_tau - roll(S_t0, 3)|`, where `roll` shifts the `x` index by `+3` cyclically, divided by the maximum of `|S_t0|`, is `<= 1.0e-9`, and `guard_pass = 1` (`checks.translation`). The measured value is `1.1e-14`. A model that translates the low westward, or that leaves it stationary, gives `1.7` and `0.84` for the same expression, and a model whose `x` dependence is the Gaussian of `x-x_c(t)` rather than the periodic argument of §3 gives `0.25`; each fails this judgment. This judgment does not read `S` or `S_fd`, which are the sentinel on this case.
...
- `test_id`: `l0_invalid_r_low_zero_xfail`
  ...
  - `pass_when`: `verdict.overall == fail and verdict.failed_checks includes 'input_guard' and checks.rejected_outputs_zero.status == pass`
  - `judgment`: with `R_low=0.0`, the `operation` returns `guard_pass = 0` (`checks.input_guard` is `fail`, which makes the case's `verdict.overall` `fail`) and writes exactly `0` into every element of `S` (`checks.rejected_outputs_zero`: no element is the setup sentinel `-999`). The captured `guard_pass` is `0`, not the setup value `-1`. A model whose clause is `R_low<0` fails this test, and so does a model that evaluates the forcing on the rejected input: the division by `R_low^2` of the Controlled Spec §3 is evaluated before the guard only if the guard is absent or late.
...
- `test_id`: `l0_invalid_l_x_zero_xfail`
  ...
  - `pass_when`: `verdict.overall == fail and verdict.failed_checks includes 'input_guard' and checks.rejected_outputs_zero.status == pass`
  - `judgment`: with `L_x=0.0`, the `operation` returns `guard_pass = 0` (`checks.input_guard` is `fail`, which makes the case's `verdict.overall` `fail`) and writes exactly `0` into every element of `S` (`checks.rejected_outputs_zero`: no element is the setup sentinel `-999`). The captured `guard_pass` is `0`, not the setup value `-1`. A model whose clause is `L_x<0` fails this test, and so does a model that evaluates the forcing on the rejected input: the division by `L_x` of the Controlled Spec §3 is evaluated before the guard only if the guard is absent or late.
