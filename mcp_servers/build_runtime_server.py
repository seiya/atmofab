...
_WORKFLOW_MODE_ENV_VARS = ("METFORGE_WORKFLOW_MODE", "METFORGE_ORCHESTRATION_ID")


def _workflow_mode_env_signal() -> str | None:
    """The workflow environment variable that puts this server under a run, if any.

    `tools/run_workflow.py` sets `METFORGE_WORKFLOW_MODE=1` in the node environment and
    the conductor adds `METFORGE_ORCHESTRATION_ID` per child. Both reach the leaf's CLI —
    and the MCP server it spawns — because they are on the leaf's DECLARED environment
    (`METFORGE_*` is allowlisted by prefix — the prefix itself lives in
    ...
    for name in _WORKFLOW_MODE_ENV_VARS:
        ...
        if not value:
            continue
        if name == "METFORGE_WORKFLOW_MODE" and value == "0":
            continue
        return name
    return None
...
_ORCHESTRATION_GATE_PROPERTIES: dict[str, Any] = {
    "orchestration_id": {
        "type": "string",
        "description": (
            "Required under the workflow (METFORGE_ORCHESTRATION_ID set, or "
            "METFORGE_WORKFLOW_MODE set to a non-empty value other than 0, in the "
...
#: The syntax-only stage `Generate.gate` runs whatever `METFORGE_SYNTAX_COMPILERS` lists, and the
...
SYNTAX_CANARY_SOURCE = "module metforge_syntax_canary\n  implicit none\nend module metforge_syntax_canary\n"
