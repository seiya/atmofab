...
def _workflow_mode_env_signal() -> str | None:
    ...
    (`METDSL_*` is allowlisted by prefix in `orchestration_runtime.LEAF_ENV_ALLOWLIST`'s
    neighbourhood) and the bwrap profile `--setenv`s each one after `--clearenv`; it is
