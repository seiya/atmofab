...
- `semantic_review.json` requires `review_method`, `decision`, `scope.model_ref`, `scope.runner_ref`, `scope.raw_refs`, and `findings`. `review_method` **must be the exact literal string `"llm_semantic_review"`** — the conductor's `--stage pre_judge` gate rejects any other value. A wrong literal terminalizes the phase `fail_closed` on your only attempt — you are not re-run to correct it.
