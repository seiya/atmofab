...
- The semantic check reconciles the runner's `diagnostics.json` against the independent `raw/` primary evidence: recompute the judged quantities from `runs/<run_id>/<node_key_safe>/raw/` and confirm they are consistent with `diagnostics.json`. Recomputation impossible, or a diagnostics/raw inconsistency (a fabricated or mismatched `diagnostics.json`), is a **semantic `fail`** — record it as a `findings[*]` with `attribution=code` (untrustworthy runner evidence) or `attribution=evidence` (raw evidence missing/malformed). The mechanical verdict trusted `diagnostics.json`; your review is the check that it deserved that trust.
- Recompute from the `raw` evidence only — reusing `diagnostics.json` as input is circular.
- Confirm the primary evidence composition against `spec.ir.yaml.io_contract.raw_requirements.required_evidence` (do not uniformly require a fixed set), and that `raw/metrics_basis.json` covers the (`test_id`, `case_id`) matrix: one entry per `test_id` of `io_contract.test_evidence_requirements` × each case named in that test's `test_predicates[].target_cases` (a multi-target test carries one entry per case, each with its own `case_id`). A missing entry / a whole-suite-only summary is a semantic `fail` (`attribution=code`).
- Confirm the physics semantically satisfies `tests.md` intent beyond the mechanical predicate (e.g. the pass is not a degenerate/trivial artifact, invariants stated in prose hold). An all-zero array is NOT by itself degenerate: a flat bed, a zero transverse momentum component and a zero-diffusivity case are legitimately zero everywhere. A semantic mismatch the predicate DSL cannot express is a `fail` with the appropriate `attribution`. (Scope: for an `infrastructure` harness node — `spec_kind: infrastructure` — "the physics" is the runner-plumbing behaviour: emitter round-trips, case fan-out, the `--cases` input guard. Confirm THAT satisfies `tests.md`; do NOT fail the node for "having no physics".)
- `semantic_review.json` requires `review_method`, `decision`, `scope.model_ref`, `scope.runner_ref`, `scope.raw_refs`, and `findings`. `review_method` **must be the exact literal string `"llm_semantic_review"`** — the conductor's `--stage pre_judge` gate rejects any other value. A wrong literal is a recoverable conformance violation: the conductor warm-resumes you to re-author it, terminalizing `fail_closed` if the budget is exhausted.
...
- The `--stage pre_judge` gate is run by the **conductor** as two deterministic substeps wrapping you (G4), not by you. You are a pure LLM semantic pass: write `semantic_review.json` with your actual `decision` (`pass`/`fail`); the deterministic `post_judge` substep authors the derived `aggregate_verdict.json` / `summary.json` / `validate_meta.json` from the host-authored `verdict.json` + the dependency set (G6), then runs `--stage pre_judge` and records the verdict + a severity `disposition` in `post_judge_meta.json`.
  - The conductor gates on both sides: the `pre_judge` substep (index 0) is a dependency-DAG readiness check (a not-yet-built+validated `dependency.all_nodes` closure fails the phase `fail_closed` before you are spawned), and the `post_judge` substep (index 3) authors the derived artifacts (G6) and runs the `--stage pre_judge` validator (orchestration-record + cross-pipeline DAG integrity, scoped to this run). A violation in your `semantic_review.json` (e.g. a wrong `review_method`), an integrity violation, and any `pre_judge` `fail` are all terminalized `fail_closed` by the conductor — you are not re-run to repair one. (You no longer write the `validate` `step_result.json` yourself.)

## Operations Rules
1. Limit the review input to only artifacts under the same `run_id`.
2. `diagnostics.json`, `perf.json` and the `raw` evidence must all exist; when any is missing the AGENTIC judge does not start. (The PURE judge does start — the host reports the absence in its excerpt and it fails with `attribution=evidence`.)
