...
- Confirm the physics semantically satisfies `tests.md` intent beyond the mechanical predicate (e.g. the pass is not a degenerate/trivial artifact, invariants stated in prose hold). A semantic mismatch the predicate DSL cannot express is a `fail` with the appropriate `attribution`. (Scope: for an `infrastructure` harness node — `spec_kind: infrastructure` — "the physics" is the runner-plumbing behaviour: emitter round-trips, case fan-out, the `--cases` input guard. Confirm THAT satisfies `tests.md`; do NOT fail the node for "having no physics".)
