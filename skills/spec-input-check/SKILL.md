...
- `docs/GLOSSARY.md` — the allowed `domain` / `family` classification vocabulary, and the `deps.yaml` declaration vocabulary (`component_id` / `profile_id`; no `infrastructure_id` — the harness is the target profile's).
- `docs/workflow/phases/phase_01_compile.md` — the node-**identity** preconditions `Compile` cannot repair by re-authoring, so they must be caught here: `spec_id` ≤ 55 characters, and **no** `infrastructure` dependency declared in `deps.yaml`, on every `spec` whatever its kind (the harness is the target profile's, `docs/SPEC.md` req. 9; a `profile` is checked at its adopter instead — check F.6). Also the `infrastructure` §5 / §5.1 public-API pin, and the rule that the runner emits every numeric judgment **already reduced to a field, so predicates do no arithmetic**.
...
1. `deps.yaml` declares the `dependencies` block, whose keys are drawn from exactly `components` / `profiles`. Both are **required in every `deps.yaml`**, an `infrastructure` spec's own included (there both are empty lists); empty lists are likewise valid for a leaf `component`, and for a `problem` whose whole `component` set comes from an adopted `profile`. An `infrastructure:` key is refused by name (check 6). Any other key under `dependencies` is a `blocker` — the closure build rejects the whole `deps.yaml` as malformed, so a typo like `component:` stops the run.
...
4. Each declared dependency is registered in `spec/registry/spec_catalog.yaml` under the matching `spec_kind`, and its files exist on disk. An unregistered or missing dependency is a `blocker` (`docs/SPEC.md` req. 11).
5. Each dependency's catalog `spec_version` satisfies the declared `version_constraint`. A violation is a `blocker`. (The harness version is the target profile's `harness.version_constraint`, which the launch gate resolves; it is not a `deps.yaml` entry.)
6. **No `infrastructure` entry** in any `deps.yaml`, whatever the spec's kind (`docs/SPEC.md` req. 9). The harness is an attribute of the target profile (`spec/targets/<target_id>.yaml` `harness`), and the host adds the target's harness as a direct dependency of every non-`infrastructure` node it builds for that target, so a node with a host-rendered runner (an `M3c` node) needs no declaration. **One or more entries is a `blocker`**: the workflow rejects such a spec at **spec-input**, before any phase runs, and rejects it for every member of a `--with-deps` closure, so a violating spec never reaches `Compile`. The two capture points emit different envelopes, so do not look for one literal string: a plain (no `--with-deps`) run rejects its target inside the conductor with `spec-input rejected: infrastructure_dependency_declared_in_deps: …`, while a `--with-deps` run resolves the closure first and reports **any** member — the target included — as `{"reason": "dependency_closure_unresolved", "reason_code": "infrastructure_dependency_declared_in_deps", …}` (the violated rule is in `reason_code`, not `reason`). A `--resume` re-resolves the node through the same gate, so an already-started run cannot be resumed past the defect either; the remedy is to delete the `infrastructure:` section and start a new run. A `profile` is the one case neither envelope applies to: it is not a run target and is expanded away before it is visited as a closure member, so an entry there is a `blocker` whose refusal arrives one phase later, at the Compile start of every ADOPTING node, as `profile_declares_infrastructure`. (Until R4-a PR-3, issue #284, the rule was the reverse — exactly one entry on every spec that builds, `infra_dep_count_invalid` — so an older spec that still carries an `infrastructure:` section is exactly the shape this check refuses.)
