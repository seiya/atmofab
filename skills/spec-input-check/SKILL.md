...
1. All three files (`controlled_spec.md`, `deps.yaml`, `tests.md`) exist at the resolved spec directory. A missing file is a `blocker`. **Except a `profile`**, which has TWO files and no `tests.md` (`docs/SPEC.md` req. 5, issue #175): nothing is generated or executed for a `profile`, so a `tests.md` present there is a `blocker` in the other direction.
2. `deps.yaml` parses as YAML; `controlled_spec.md` (and `tests.md`, where the kind has one) parse as Markdown with a readable meta block.
3. At most one `tests.md` exists for the spec (`docs/SPEC.md`: only 1 file per spec), and exactly one on every kind but `profile`.
...
- `profile`: sections 1–5 (Target `component` and compatibility range; Application conditions; Parameter constraints; Fallback rules; Traceability). There is no tests-reference section — a `profile` has no `tests.md`.
...
6. Where `tests.md` names case identifiers literally (rather than deriving them from a sweep rule), each must survive the `Compile` case gate: at most 64 characters, matching `[A-Za-z0-9._][A-Za-z0-9._-]*` (a leading `-` is refused: the id reaches the runner's argv), containing no `..`, and pairwise distinct (`docs/workflow/phases/phase_01_compile.md`). A `case_id` is concatenated into the per-case snapshot path and rendered as a Fortran `select case` label, so an over-long one is truncated until no label can ever match and every run aborts despite compiling cleanly. Report a violation as a `warning`, since `Compile` — not `tests.md` — fixes the final `case_id` set; state that the check applies only to the literally-named ids.

### F. deps.yaml and dependency resolvability
1. `deps.yaml` declares the `dependencies` block, whose keys are drawn from exactly `components` / `profiles` / `infrastructure`. `components` and `profiles` are **required in every `deps.yaml`**, an `infrastructure` spec's own included (there both are empty lists); empty lists are likewise valid for a leaf `component`, and for a `problem` whose whole `component` set comes from an adopted `profile`. `infrastructure` is required with exactly one entry in every spec that BUILDS (check 6) and absent (or empty) in an `infrastructure` spec's own and in a `profile`'s. Any other key under `dependencies` is a `blocker` — the closure build rejects the whole `deps.yaml` as malformed, so a typo like `infrastructures:` does not degrade to "no harness dependency", it stops the run.
2. A `problem` spec has a direct `component` set — declared in its own `components`, selected by a `profile` it adopts, or both (`docs/SPEC.md` req. 10). A `problem` with NEITHER is a `blocker`. A `component` that appears in both places is also a `blocker`: a `component` has exactly one source, and the closure resolver refuses the duplicate (`profile_component_declared_twice`).
...
6. **Exactly one `infrastructure` entry** in every spec that BUILDS. Two kinds are outside that rule and they are outside it differently: an `infrastructure` spec declares none (it authors its own self-test runner) and the deterministic gates are SILENT at every count, while a `profile` spec is bounded to ZERO (it is a host-resolved selection policy that builds nothing, `docs/SPEC.md` req. 9) and one or more IS a refusal — reported as `profile_declares_infrastructure` at the Compile start of every ADOPTING node, not at the profile itself, which no run ever targets. The single entry makes the node an `M3c` node: its runner is host-rendered glue over the certified harness plumbing plus a leaf-authored `<spec_id>_checks` module, and no other runner path exists. **Both zero and more than one are a `blocker`** (`docs/workflow/phases/phase_01_compile.md`, `docs/SPEC.md` req. 9): the workflow rejects such a spec at **spec-input**, before any phase runs, and rejects it for every member of a `--with-deps` closure, so a violating spec never reaches `Compile`. The two capture points emit different envelopes, so do not look for one literal string: a plain (no `--with-deps`) run rejects its target inside the conductor with `spec-input rejected: …`, while a `--with-deps` run resolves the closure first and reports **any** member — the target included — as `{"reason": "dependency_closure_unresolved", "reason_code": "infra_dep_count_invalid", …}` (the violated rule is in `reason_code`, not `reason`). A `--resume` re-resolves the node through the same gate, so an already-started run cannot be resumed past the defect either; the remedy is to fix `deps.yaml` and start a new run. The `spec_kind` that decides the exemption is read from `spec_catalog.yaml`, not from `deps.yaml`'s own `spec_kind` field, so a mismatch between the two normally does not change the verdict — report the mismatch itself under check C.5. It DOES decide when the registry is unreadable, and then only to grant the exemption — a rejection resting on it is reported as the registry defect instead. A `spec_id` registered under more than one `spec_kind` (forbidden by `docs/SPEC.md` req. 4) is reported as the registry defect only when it is the `--with-deps` TARGET; reached as a closure member it is judged by the requiring edge's kind, and on a plain run the conductor takes the FIRST matching catalog entry and never notices the duplicate at all. So report such a duplicate as a `blocker` in its own right — the workflow catches it in one of three positions. For an `infrastructure` spec the deterministic gates are silent at every count, so an `infrastructure` spec that nonetheless declares an `infrastructure` entry is a `warning`, not a `blocker`. A `profile` is the opposite case and neither envelope above applies to it: it is not a run target and is expanded away before it is visited as a closure member, so both spec-input capture points are unreachable for one — an entry there is a `blocker` whose refusal arrives one phase later, at an adopting node.
7. `controlled_spec.md §4` (for a `problem`) names the same `profile_id`s that `deps.yaml` declares, and the `component_id`s of its direct set — those `deps.yaml` declares directly PLUS those the adopted `profile`'s own `deps.yaml` selects — neither side referencing an `id` the other omits. A discrepancy is a `blocker` (contradiction between the prose contract and the machine-readable dependency set).
8. For a `profile` spec, the `target component_id` in `controlled_spec.md §1` is exactly what its own `deps.yaml#components` declares. Compatibility between an adopted `profile` and the components an adopting `problem` declares directly is the host resolver's job (`profile_component_declared_twice` / `profile_component_conflict`), not a prose check here (cross-spec consistency → report what is checkable from the given files; mark unverifiable items `info`).
