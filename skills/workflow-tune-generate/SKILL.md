...
- Keep `target_lowering_plan.parallelization` an object whose `model` member names the parallel model (`"none"` when nothing is parallelized). The `Generate.gate` parallel-directive floor reads the model from that member (`docs/IMPL_PLAN_SPEC.md` §3), so a model named under another key disables the floor for the variant. A brand-new member inside an optional plan section is fine.
