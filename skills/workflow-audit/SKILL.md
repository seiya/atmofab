...
- **`## Pure-leaf A/B metrics`** — per node, the attempt and repair-turn counts and the summed usage of each pure leaf. Detail (per attempt): the `*_meta.json` next to the node's artifact (`docs/WORKSPACE_LAYOUT.md`).
