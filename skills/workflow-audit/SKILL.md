...
- **`## Phase state failures`** — every `fail` / `fail_closed` transition with its `reason_code`, node and step, and the `fail_closed` instant. Detail: the matching line of `phase_state_log.jsonl`.
