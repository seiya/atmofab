...
- **`## agent_runs summary`** — the run count per status, the rows missing `finished_at`, and `Repeated substeps` — every `(node_key, step, substep)` with more than one attempt, with the status sequence (a `step` row such as `build` has no substep and is listed by its step). A run rejected at terminal validation (`agent_runs_invalid.jsonl`) is counted under its status, typically `fail`.
