...
- **`## Pure-leaf A/B metrics`** — per node, the attempt and repair-turn counts and the summed usage of each pure leaf. A row's `result=` is the LOOP outcome (a schema-valid document was obtained); a reviewer row's `verdict=` is what that document decided, read from the projection beside the record (`ir_meta.json` / `source_meta.json#verification_status`, `semantic_review.json#decision`) — so a rejecting reviewer reads `result=pass, verdict=fail`. When a retry route has since revoked the artifact, the reviewer's own decision is still what `verdict=` shows (read from `prior_verification_status`) and the route's reason and agent run follow it — `verdict=pass (revoked: validate_execute_post_execute_violation by arid …)` is a reviewer that ACCEPTED and a downstream failure that reopened the phase, not a rejection. Resolve that arid against THIS run's `agent_runs.jsonl` before attributing the revocation to this run: the generate rows list every source directory under the node's pipeline, which the node's orchestrations share, and a later run revokes an earlier run's artifact too, by a conductor route on a cold re-run or by an operator's `revoke-artifact` by hand (`docs/RUNBOOK.md` §3-1) — an arid this run does not hold means the revocation is another run's. `verdict=none` means no projection was written — the reviewer loop ended with no accepted document, by budget exhaustion or a transport death, and the row's `failure:` says which — for `generate` / `validate`; on `compile` the same end reads `verdict=pending`, the status the producer wrote before the reviewer ran. Count rejections by `verdict=`, never by `result=` and never by the revocation. Detail (per attempt): the `*_meta.json` next to the node's artifact (`docs/WORKSPACE_LAYOUT.md`).
- **`## agent_runs summary`** — the run count per status, the rows missing `finished_at`, and `Repeated substeps` — every `(node_key, step, substep)` with more than one attempt, one line per attempt with its status and `agent_run_id` in start order (a `step` row such as `build` has no substep and is listed by its step). Detail: `launches/<agent_run_id>.reply.txt` of each attempt. A run rejected at terminal validation (`agent_runs_invalid.jsonl`) is counted under its status, typically `fail`.
...
3. **Sandbox enforcement** — each violation with its reason and `agent_run_id`, or the statement that none was recorded (naming which of the three negative sentences the tool printed: directory absent, directory present with no record, or records of another kind only).
...
- **`codex_cli`**: `<orchestration_meta.json#codex_workflow_home>/<codex_lineage_id>/sessions/YYYY/MM/DD/rollout-*-<agent_session_id>.jsonl` — the thread's own lineage home under the orchestration's durable container `~/.atmofab/homes/<orchestration_id>/codex/` (`docs/RUNBOOK.md` §"The operator-private root"; `tools/prune_workflow_homes.py` is the only thing that removes it). Both `agent_session_id` and `codex_lineage_id` are read from the leaf's `session_run_index.json` row, which maps `agent_run_id` to them (the launch response `launches/<agent_run_id>.response.json` carries `codex_lineage_id` too); every attempt of one thread shares the lineage id, so a warm repair's rollout is in the same file tree as the attempt it resumed. A run recorded before issue #245 has rows with no `codex_lineage_id` and its rollouts directly under `<codex_workflow_home>/sessions/…` (one home per orchestration). `~/.codex/sessions` holds the operator's own sessions, not a leaf's.
