...
> — the backend CLI's credential/session home, and since issue #64 `~/.met-forge`, under
> which the durable private homes live — so the guard rejects these reads fail-closed
> whenever `METFORGE_WORKFLOW_MODE=1` (policies `forbid_backend_credential_direct_read` /
...
- **A leaf can be KILLED at the per-leaf cap.** Such a leaf reports the same
  ...
  `METFORGE_LEAF_TIMEOUT_SECONDS` (7200 s by default — the cap plus the
