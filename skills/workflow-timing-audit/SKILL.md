...
| per-leaf full transcript (per-TURN detail: thinking split, tool time) | `<projects-root>/<cwd-slug>/<agent_run_id>.jsonl` (`<projects-root>` is `~/.claude/projects` for a pure leaf, which has no private home since Z4 — issue #171; issue #63 had put an agentic leaf's under `orchestration_meta.json#claude_workflow_home` + `/projects`, and the bundled script still searches that AND `~/.claude/projects`, private first and resolved per run id rather than either/or, because a run resumed across either migration has leaves in both — pass one explicitly with `--project-dir` to override the pair) (`<cwd-slug>` = repo abs-path with `/`→`-`; the leaf `agent_session_id` == `agent_run_id` == filename) |
...
and the script leaves such a row alone), printing how many rows it corrected and how many it
could not decide.
