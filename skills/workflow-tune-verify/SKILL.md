...
description: Use this when running the verify of the Tune stage and evaluating the physics passing, quality conditions, and performance objective function of a candidate lowering-plan (`target_lowering_plan`) variant to judge whether to adopt it. It applies to fixing the `best impl` and judging the move to regression.
---
...
## Scope
- the trial-result evaluation of a candidate lowering-plan (`target_lowering_plan`) variant
...
2. Among the adoption candidates, fix the variant lowering plan with the maximum objective function as the `best impl`.
