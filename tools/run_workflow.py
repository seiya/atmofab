...
def _claim_lock_path(repo_root: Path, kind: str, key: str) -> Path:
    ...
    # `METFORGE_START_CLAIM_ROOT` relocates the whole set. It exists so a caller that
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    # NOTE (M-F): the `--generate-executor` flag and the `METFORGE_GENERATE_EXECUTOR` env var were
...
def _run_main(
    ...
    *,
    ...
    # METFORGE_GENERATE_EXECUTOR env were deleted; a cold run that still passes the flag fails at
    ...
    # Base env shared by every node. METFORGE_ORCHESTRATION_ID / TMPDIR /
    ...
    base_env["METFORGE_WORKFLOW_MODE"] = "1"
    base_env["METFORGE_WORKFLOW_EXEC_MODE"] = workflow_mode
...
def _format_event_human(payload: dict[str, Any], *, elide_detail: bool = True) -> str | None:
    ...
    if status == "info" and event == "leaf_timeout":
        ...
        return (f"    [warn   ] leaf timeout in {phase}.{substep}: no answer after {elapsed}s "
                f"(cap {cap}s, METFORGE_LEAF_TIMEOUT_SECONDS) — process group killed, "
...
def _run_node(
    *,
    ...
    env = dict(base_env)
    env["METFORGE_ORCHESTRATION_ID"] = orchestration_id
    ...
    try:
        ...
        ok_output: dict[str, Any] = {
            ...
            "until_phase": until_phase,
            "workflow_mode": workflow_mode,
            "metforge_workflow_mode": env["METFORGE_WORKFLOW_MODE"],
            "metforge_workflow_exec_mode": env["METFORGE_WORKFLOW_EXEC_MODE"],
            "workflow_status": workflow_status,
