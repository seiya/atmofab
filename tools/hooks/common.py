...
WORKFLOW_HOMES_ROOT_ENV = "METFORGE_WORKFLOW_HOMES_ROOT"
...
def workflow_private_backend_homes(repo_root: Path | None,
    ...
    sandbox (`METFORGE_ORCHESTRATION_ID`, AUTHORED by `workflow_conductor._child_env`
...
def protected_host_read_roots(repo_root: Path | None = None,
    ...
    `METFORGE_WORKFLOW_HOMES_ROOT` puts them), every backend credential home the sandbox
    ...
    conductor always sets `METFORGE_ORCHESTRATION_ID`.
    """
...
def _workflow_orchestration_id() -> str | None:
    ...
    `METFORGE_ORCHESTRATION_ID` only — never the hook payload's copy, which
    ...
    """
    raw = os.environ.get("METFORGE_ORCHESTRATION_ID")
...
def evaluate_common_policy(hook_input: HookInput) -> HookDecision:
    ...
    workflow_mode_val = os.environ.get("METFORGE_WORKFLOW_MODE", "0").strip()
    ...
    if workflow_mode_val == "1":
        ...
        # An agent cannot bypass this by using METFORGE_WORKFLOW_MODE=0 prefix
...
def _claude_project_slug(repo_root: Path) -> str:
    ...
    /home/<user>/work/met-forge → -home-<user>-work-met-forge.
    """
