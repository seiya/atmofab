...
WHAT IT DOES NOT DECLARE: `runner_render` and the language half of `control_file`. The host does
not render a runner over a `cuda_cpp` harness or author a `cuda_cpp` node's build control file
yet, so a `cuda_cpp` physics node is refused at launch
(`target_profile.toolchain_servable_reasons`), and only an `infrastructure` node — the harness,
whose leaf authors both — can run.
"""
