...
def _render_pure_leaf_row(label: str, row: dict[str, Any], lines: list[str]) -> None:
    ...
    # projection file was not written (budget exhaustion), printed as `none`. A revocation is
    # rendered BESIDE the verdict, not in its place: the retry route's decision about the
    # artifact is not the reviewer's decision, and the reference run held one of each under
    # the same `revoked` status.
    verdict_str = ""
    if "verdict" in row:
