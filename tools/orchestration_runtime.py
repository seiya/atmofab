...
# flags an over-budget reply as telemetry by default; ATMOFAB_ENFORCE_REPLY_BUDGET=1 makes it
...
def _resolve_dependency_facts(
    ...
    When the CONSUMER's language declares ``signatures`` (issue #289, R4-b PR-3: its backend is
    what reads a published interface out of a certified source), each fact additionally carries
    ``published_operations`` — ``[{operation, interface, argument_order}]`` extracted from
    the dependency's CERTIFIED model source (the exact source Build links, via
    ...
    ``published_operations`` and keeps its pipeline/run/verdict fact. A consumer whose language
    declares no ``signatures`` gets no interfaces — never another language's reading of them.

    FALLBACK: when a ``component/`` dep's ``operations`` is empty / absent — an IR
    ...
    try:
        ...
        # Surface dependency call-site interfaces only when the CONSUMER's language can read
        # them: its `signatures` backend is what extracts a published interface from a
        # certified source (issue #289, R4-b PR-3). A consumer language without one gets the
        # pipeline / run / verdict facts only — never another language's reading of the source.
        consumer_language = (resolver.target.toolchain["language"]
                             if resolver.target is not None else None)
        consumer_signatures = (
            backend_registry.capability_module("language", consumer_language, "signatures")
            if consumer_language
            and backend_registry.provides("language", consumer_language, "signatures")
            else None)
        for entry in direct_deps:
            ...
            if consumer_signatures is not None and not is_infra_dep:
                try:
                    ...
                    if source_text is not None:
                        ...
                        if is_component_dep and dep_spec_id:
                            if not enumerated:
                                ...
                                # generate gate (the language's `validate_dependency_operations`); a
                                ...
                                op_names = consumer_signatures.prefixed_procedures(
                                ...
                                declared_unresolved = [
                                    ...
                                    if consumer_signatures.published_interface(
                                        source_text, op) is None
                                ]
                        ...
                        for op in op_names:
                            iface = consumer_signatures.published_interface(source_text, op)
                            if iface is None:
                                continue
...
def _dependency_ready(
    ...
    with _orchestration_meta_exclusive_lock(repo_root, orchestration_id):
        ...
        if recomputed is None:
            ...
            if os.environ.get("ATMOFAB_DEP_READINESS_ALLOW_PERSISTED_FALLBACK") != "1":
...
def _builds_in_source(build_system: str | None) -> bool:
    """Whether the target build system builds in the source tree, so its build and quality-check
    command logs legitimately land beside the control file (the cross-phase placement above).

    Asked of the build system's `control_file` backend (`BUILDS_IN_SOURCE`, issue #289 R4-b
    PR-3) rather than compared against one spelling. A build system this repository has no
    control-file backend for answers False: the cross-phase grant is the exception, and an
    unknown build system does not earn it."""
    value = str(build_system or "")
    if not backend_registry.provides("build_system", value, "control_file"):
        return False
    try:
        module = backend_registry.capability_module("build_system", value, "control_file")
...
# `ATMOFAB_*` is passed by PREFIX, not by enumeration. The namespace is repo-owned BY
...
# `leaf_env_from({"ATMOFAB_ANYTHING": "x"})` forwards it. Nothing stops an operator
# exporting a name under this prefix.
#
# ACCEPTED RESIDUE, therefore, stated rather than argued away: a `ATMOFAB_*` value an
...
LEAF_ENV_ALLOWED_PREFIXES = ("ATMOFAB_",)

# Inside the prefix but deliberately dropped. `ATMOFAB_HOME` is the deprecated alias for
...
LEAF_ENV_PREFIX_EXCEPTIONS = ("ATMOFAB_HOME",)
...
def _profile_child_env(child_env: Mapping[str, str] | None, *,
    ...
    hand the leaf a `ATMOFAB_CHILD_AGENT_RUN_ID` naming a different run, and the whole
    ...
    """
    per_launch = (("ATMOFAB_ORCHESTRATION_ID", orchestration_id),
                  ("ATMOFAB_CHILD_AGENT_RUN_ID", agent_run_id))
...
def render_bwrap_command(
    *,
    ...
    for _id_name, _id_field in (("ATMOFAB_ORCHESTRATION_ID", "orchestration_id"),
                                ("ATMOFAB_CHILD_AGENT_RUN_ID", "agent_run_id")):
...
def _live_preflight_mode() -> str:
    """Return the operation mode from the value of ATMOFAB_ORCHESTRATION_ENFORCE_LIVE_PREFLIGHT.
    ...
    """
    raw = os.environ.get("ATMOFAB_ORCHESTRATION_ENFORCE_LIVE_PREFLIGHT", "").strip().lower()
...
def _live_preflight_ttl_seconds() -> int:
    """Read ATMOFAB_PREFLIGHT_TTL_SECONDS and return a non-negative integer.

    Return PREFLIGHT_TTL_DEFAULT_SECONDS when unset or an invalid value.
    """
    raw = os.environ.get("ATMOFAB_PREFLIGHT_TTL_SECONDS", "").strip()
    if not raw:
...
def _build_dependency_surface_facts(surface: Any) -> str:
    ...
    lines = [
        ...
        "controlled_spec §5 operation_id(s) verbatim, AND its §5.1 signatures / "
        "module_parameters (issue #153 PR-2 — a component pins its full ABI, not only names); a "
...
def prepare_launch_request_payload(request_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.setdefault("workflow_mode", os.environ.get("ATMOFAB_WORKFLOW_EXEC_MODE", "dev"))
...
def _evaluate_reply_budget(
    ...
    *,
    ...
    appends an audit entry on an over-budget reply. With ATMOFAB_ENFORCE_REPLY_BUDGET=1
    ...
    if os.environ.get("ATMOFAB_ENFORCE_REPLY_BUDGET") == "1":
        raise ValueError(
            f"record-agent-run: child reply is {n} chars, over the {REPLY_BUDGET_CHARS}-char budget "
            f"(ATMOFAB_ENFORCE_REPLY_BUDGET=1). Re-launch the child with a terse final message — a status "
...
def _probe_codex_home_writable() -> dict[str, Any]:
    ...
    legacy = os.environ.get("ATMOFAB_HOME")
    if (isinstance(canonical, str) and canonical.strip()
        ...
        return {
            ...
            "detail": "CODEX_HOME and deprecated ATMOFAB_HOME conflict",
        }
    ...
    source = (
        ...
        else ("env:ATMOFAB_HOME (deprecated)" if isinstance(legacy, str) and legacy.strip()
              else "default:~/.codex")
    )
    codex_home = (
        Path(raw).expanduser()
...
def _require_secure_home_ancestor(path: Path, label: str, *, require_private: bool) -> None:
    ...
    `ATMOFAB_WORKFLOW_HOMES_ROOT` with no stated precondition — fail EVERY launch after a
    plain `mkdir` created the root 0755 under the default umask; and it made a mode drift
    on `~/.atmofab/homes` (a backup restored without permissions) refuse every launch of
...
def _create_workflow_backend_home(repo_root: Path, orchestration_id: str,
    ...
    # the failure mode that costs most. Without one, `~/.atmofab` may legitimately not
    ...
    if override_used:
        ...
        ancestors.append((root, True))          # ~/.atmofab/homes
...
def _probe_bwrap_sandbox() -> tuple[list[dict[str, Any]], bool]:
    ...
    `ATMOFAB_ORCHESTRATION_ASSUME_BWRAP=1` is a **test-only** affordance: it bypasses the
    ...
    assume = os.environ.get("ATMOFAB_ORCHESTRATION_ASSUME_BWRAP", "").strip().lower()
...
def _probe_http_provider(
    ...
    *,
    ...
    `ATMOFAB_HTTP_PREFLIGHT_SKIP_REACHABILITY=1` drops only (3), for an offline preflight."""
    import urllib.error
...
def probe_execution_platform(
    *,
    ...
    session_policy = {
        "allow_step_agent_launch": os.environ.get("ATMOFAB_ALLOW_STEP_AGENT_LAUNCH", "1").strip().lower()
        not in {"0", "false", "no"},
        "allow_substep_agent_launch": os.environ.get(
            "ATMOFAB_ALLOW_SUBSTEP_AGENT_LAUNCH", "1"
        ).strip().lower()
        not in {"0", "false", "no"},
    }
    return {
    ...
    *,
    ...
    with _runs_jsonl_exclusive_lock(repo_root, orchestration_id):
        ...
        if not has_orchestration_running_entry:
            ...
            # has no launch, so it is threaded in here.
...
def record_agent_run(
    ...
    if role_token in {"step", "substep"}:
        ...
        # ATMOFAB_ENFORCE_REPLY_BUDGET=1). Evaluated before the terminal append so a
...
def finalize_child(
    ...
    *,
    ...
    if (
        ...
        and os.environ.get("ATMOFAB_ENFORCE_REPLY_BUDGET") == "1"
    ):
        ...
        raise ValueError(
            ...
            f"budget (ATMOFAB_ENFORCE_REPLY_BUDGET=1). Re-launch the child with a terse final message — a "
...
def _rewrite_orchestration_run_row(
    ...
    *,
    ...
    (not append) under the agent_runs.jsonl exclusive lock rather than appending,
    so it never trips the duplicate guard. `should_rewrite` decides whether the matched row needs changing (for
...
def main(argv: list[str] | None = None) -> int:
    ...
    _RECORD_LAUNCH_REQUEST_HELP = (
        ...
        "For a DETERMINISTIC step/substep launch, one of allowed_output_paths|required_outputs|output_refs "
        "must be provided as a file-path list; the runtime validates each path against the phase contract's "
        "outputs. (It also validated them against the capability's write_roots until issue #171 PR-2, which "
        "deleted the capability document.) A PURE launch must leave allowed_output_paths empty -- the host "
        "writes every artifact after the child window closes. allowed_file_tool_paths is accepted and IGNORED: "
        "it named a leaf's Edit/Write grant, and no leaf holds either tool. "
