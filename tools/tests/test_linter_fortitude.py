...
2. **Resolution against the INSTALLED build** — runs the tool. These are the ones that matter:
   ...
   metforge-enforcement-change` judgment rule 2: a machine without the tool is a machine that
...
_CLEAN_SOURCE = """module metforge_probe_model
  use, intrinsic :: iso_fortran_env, only: real64
  implicit none
  private
  public :: metforge_probe__op
contains
  subroutine metforge_probe__op(x, y)
    real(real64), intent(in) :: x
    real(real64), intent(out) :: y
    y = x
  end subroutine metforge_probe__op
end module metforge_probe_model
"""
...
#: nothing here.
_DEFECTIVE_SOURCE = """module metforge_probe_bad
  use iso_fortran_env, only: real64
  implicit none
contains
  subroutine metforge_probe_bad__op(x, y)
    real(8), intent(in) :: x
    real(8), intent(out) :: y
    y = x{padding}
  end subroutine metforge_probe_bad__op
end module metforge_probe_bad
...
class ResolutionAgainstTheInstalledBuildTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        (self.dir / "metforge_probe_model.f90").write_text(_CLEAN_SOURCE)
    ...
    def test_every_declared_code_is_selectable_on_the_installed_build(self) -> None:
        ...
        """
        completed = _run(list(lint.check_argv("metforge_probe_model.f90")), self.dir)
    ...
    def test_the_resolved_rule_set_equals_the_declared_one(self) -> None:
        ...
        """
        self.assertEqual(_resolved_rule_codes(self.dir, "metforge_probe_model.f90"),
    ...
    def test_a_neighbouring_config_file_cannot_change_the_verdict(self) -> None:
        ...
        unchanged.
        """
        (self.dir / "metforge_probe_bad.f90").write_text(_DEFECTIVE_SOURCE)
        without_config = _run(list(lint.check_argv("metforge_probe_bad.f90")), self.dir)
        ...
        with_config = _run(list(lint.check_argv("metforge_probe_bad.f90")), self.dir)
        ...
        unisolated = [arg for arg in lint.check_argv("metforge_probe_bad.f90")
                      if arg != "--isolated"]
    ...
    def test_an_allow_directive_suppresses_nothing(self) -> None:
        ...
        (self.dir / "metforge_probe_bad.f90").write_text(blanket)
        suppressible = {"C122", "C131", "PORT011"}

        declared = _run(list(lint.check_argv("metforge_probe_bad.f90")), self.dir)
        ...
        without_flag = [a for a in lint.check_argv("metforge_probe_bad.f90")
    ...
    def test_a_plain_implicit_none_needs_no_directive(self) -> None:
        ...
        """
        self.assertEqual(
            _run(list(lint.check_argv("metforge_probe_model.f90")), self.dir).returncode, 0)
        (self.dir / "metforge_probe_model.f90").write_text(
...
class ProseCouplingTests(unittest.TestCase):
    ...
    Four leaf-read sites name individual codes, and `.claude/skills/metforge-enforcement-change`
