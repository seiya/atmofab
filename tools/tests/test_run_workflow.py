...
def setUpModule() -> None:
    ...
    under `~/.met-forge/start_claims/` on every run — thousands of dentries that nothing
    ever reaps. Production leaves the variable unset.
    """
    global _CLAIM_ROOT_TMPDIR, _SAVED_CLAIM_ROOT
    _SAVED_CLAIM_ROOT = os.environ.get("METFORGE_START_CLAIM_ROOT")
    _CLAIM_ROOT_TMPDIR = tempfile.TemporaryDirectory(prefix="metforge_claims_")
    os.environ["METFORGE_START_CLAIM_ROOT"] = _CLAIM_ROOT_TMPDIR.name


def tearDownModule() -> None:
    if _SAVED_CLAIM_ROOT is None:
        os.environ.pop("METFORGE_START_CLAIM_ROOT", None)
    else:
        os.environ["METFORGE_START_CLAIM_ROOT"] = _SAVED_CLAIM_ROOT
...
class RunWorkflowTests(unittest.TestCase):
    ...
    def test_claim_root_is_relocatable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            saved = os.environ.get("METFORGE_START_CLAIM_ROOT")
            os.environ["METFORGE_START_CLAIM_ROOT"] = str(claim_root)
            try:
                ...
                if saved is None:
                    ...
                    os.environ["METFORGE_START_CLAIM_ROOT"] = saved
    ...
    def test_ambient_env_executor_is_inert(self) -> None:
        # M-F: METFORGE_GENERATE_EXECUTOR was removed and is fully inert. A stale ambient value (even
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.environ["METFORGE_GENERATE_EXECUTOR"] = "legacy"  # stale ambient value: must be inert
            try:
                code, out = self._resume_capture(repo_root, oid, [])
                self.assertEqual(code, 0, out)
            finally:
...
class StdoutFormatTests(unittest.TestCase):
    ...
    def test_format_event_human_known_events(self) -> None:
        ...
        self.assertEqual(
            ...
            "(cap 7200s, METFORGE_LEAF_TIMEOUT_SECONDS) — process group killed, "
            "phase fails closed",
        )
