...
class _StandaloneServerEnvMixin:
    ...
    (`METFORGE_WORKFLOW_MODE` / `METFORGE_ORCHESTRATION_ID` in its environment), so without
    ...
    def setUp(self) -> None:
        ...
        os.environ.pop("METFORGE_WORKFLOW_MODE", None)
...
class OrchestrationGateFailClosedTests(unittest.TestCase):
    ...
    def test_workflow_mode_refuses_gated_tool_without_orchestration_id(self) -> None:
        for tool in self.GATED_TOOLS:
            with self.subTest(tool=tool):
                os.environ["METFORGE_WORKFLOW_MODE"] = "1"
                with self.assertRaises(ValueError) as ctx:
                ...
                self.assertIn("METFORGE_WORKFLOW_MODE", str(ctx.exception))

    def test_orchestration_id_env_alone_also_refuses(self) -> None:
        # Either signal suffices: the conductor sets METFORGE_ORCHESTRATION_ID per child
        # on top of the run-wide METFORGE_WORKFLOW_MODE.
        os.environ["METFORGE_ORCHESTRATION_ID"] = "orch_x"
        with self.assertRaises(ValueError) as ctx:
            self._call("run_linter", self._args("run_linter"))
        self.assertIn("METFORGE_ORCHESTRATION_ID", str(ctx.exception))
    ...
    def test_workflow_mode_off_is_not_workflow_mode(self) -> None:
        ...
        os.environ["METFORGE_WORKFLOW_MODE"] = "0"
    ...
    def test_detect_build_system_is_refused_under_the_workflow(self) -> None:
        ...
        os.environ.pop("METFORGE_WORKFLOW_MODE")
...
class OrchestratedEnvAllowlistTests(unittest.TestCase):
    ...
    def test_under_the_workflow_the_root_must_be_the_servers_own_checkout(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"METFORGE_WORKFLOW_MODE": "1"}):
            with self.assertRaises(ValueError) as ctx:
