...
<backend>` under `~/.met-forge/homes` — because it holds the leaf's only conversation
...
directories into the operator's real `~/.met-forge/homes` and leave them there — mixed in
...
TWO LAYERS, and the second is the one that actually holds:
  ...
  1. REDIRECT. A function-scoped autouse fixture points `METFORGE_WORKFLOW_HOMES_ROOT` at
  ...
  2. ENFORCE, BEFORE THE FACT. A session-scoped guard wraps `_workflow_homes_root` — the
     ...
     real directories in the operator's `~/.met-forge/homes` — permanent, unverifiable
...
is about which names the tree reads, not about that commit's text:
  ...
  every `METFORGE_*` name found in the tree, plus
...
`METFORGE_ORCHESTRATION_ENFORCE_LIVE_PREFLIGHT=1` 84 failed **and 482s**, because the tests
then run the real probes; `CODEX_HOME` 10; `METFORGE_HOME` 3; `METFORGE_ENFORCE_REPLY_BUDGET`
...
paragraph said "the 17 `METFORGE_*` names the tree reads" for four commits; its replacement
...
host names reach a leaf — so a `METFORGE_*` knob added later is neutralized without anyone
...
def pytest_addoption(parser) -> None:
    parser.addoption(
        ...
        help="do not strip the operator's METFORGE_* / CODEX_HOME / CLAUDE_CONFIG_DIR "
...
def pytest_configure(config) -> None:
    ...
    run — `METFORGE_ORCHESTRATION_ENFORCE_LIVE_PREFLIGHT=1` is the sharp case, worth 84
...
def _forbid_isolated_homes_in_operator_secret_root():
    """Fail any test about to resolve the isolated-homes root to the real `~/.met-forge`."""
