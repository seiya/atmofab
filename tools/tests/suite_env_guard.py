...
# The backend configuration homes, which carry no `METFORGE_` prefix. `CODEX_HOME` cost 10
...
def operator_env_names_to_strip(environ) -> list[str]:
    ...
    that decides which host names reach a leaf — so a `METFORGE_*` knob added later is
...
def restore_operator_env(environ) -> None:
    ...
    — the four modules that `setdefault` METFORGE_DEP_READINESS_ALLOW_PERSISTED_FALLBACK at
