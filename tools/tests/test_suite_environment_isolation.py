...
# with `METFORGE_ORCHESTRATION_ID` exported got a failure belonging to no change.
...
def _environment_names_read_by(repo_root: Path) -> set[str]:
    ...
    "METFORGE_ORCH_LIVENESS_TTL_SECONDS"` then `os.environ.get(_LIVENESS_TTL_ENV)`. Six names
...
class OperatorEnvironmentIsolationTests(unittest.TestCase):
    ...
    def test_no_ambient_operator_name_survives_into_a_running_test(self) -> None:
        ...
        TWO claims, because "no `METFORGE_*` name is set during a test" is false and would
        ...
        2. Every `METFORGE_*` name that IS set during a test is one the SUITE set, and is
        ...
        for name, operator_value in STRIPPED_OPERATOR_ENV.items():
            if name in SUITE_OWNED_ENV and os.environ.get(name) == operator_value:
                ...
                # to use — `METFORGE_DEP_READINESS_ALLOW_PERSISTED_FALLBACK=1` is the suite's
    ...
    def test_the_ratchet_reports_a_name_nobody_has_claimed(self) -> None:
        ...
        self.assertEqual(
            undeclared_operator_env_names({**declared, "METFORGE_A_NEW_KNOB": "1"}),
            {"METFORGE_A_NEW_KNOB"})
        self.assertEqual(
    ...
    def test_the_strip_removes_the_name_and_records_what_it_removed(self) -> None:
        ...
        environ = {"METFORGE_A_KNOB": "1", "CODEX_HOME": "/tmp/x", owned: "9", "PATH": "/b"}
        removed = suite_env_guard.strip_operator_env(environ)
        self.assertEqual(removed, sorted(["CODEX_HOME", "METFORGE_A_KNOB", owned]))
        self.assertEqual(environ, {"PATH": "/b"})
        self.assertEqual(suite_env_guard.STRIPPED_OPERATOR_ENV,
                         {"METFORGE_A_KNOB": "1", "CODEX_HOME": "/tmp/x", owned: "9"})
        suite_env_guard.restore_operator_env(environ)
        self.assertEqual(environ, {"METFORGE_A_KNOB": "1", "CODEX_HOME": "/tmp/x",
                                   owned: "9", "PATH": "/b"})
        self.assertEqual(suite_env_guard.STRIPPED_OPERATOR_ENV, {})
    ...
    def test_the_rule_is_read_from_the_leaf_env_prefix_constant_not_copied(self) -> None:
        ...
        reach a leaf; a `METFORGE_*` knob added later must be neutralized here without
        anyone remembering this file exists. Pinning the string `"METFORGE_"` in a second
        ...
        sample = {"METFORGE_ORCHESTRATION_ID": "x", "ZZTOP_KNOB": "y", "PATH": "/b"}
        self.assertEqual(operator_env_names_to_strip(sample), ["METFORGE_ORCHESTRATION_ID"])
    ...
    def test_the_backend_configuration_homes_are_covered_too(self) -> None:
        """`CODEX_HOME` carries no `METFORGE_` prefix and cost 10 failures when exported.
    ...
    def test_every_environment_name_the_tree_reads_is_stripped_or_declared(self) -> None:
        ...
        The first version of this test collected `METFORGE_*` LITERALS with a regex and
        asked whether the strip covers them. Two reviewers found the same thing from
        different angles: the regex harvests only names beginning with `METFORGE_` and the
        rule covers exactly names beginning with `METFORGE_`, so the equality was true by
        construction. Adding `GEMINI_CONFIG_DIR` or `METFORGE2_NEW_KNOB` to the tree — both
        ...
        # first version anchored that root on `METFORGE_ORCHESTRATION_ID`, which is also read
        # under `tools/`, so dropping the whole `mcp_servers` root survived — the exact
        # silent shrinkage this block exists to stop.
        for name, where in (("METFORGE_HOOK_REPO_ROOT", "tools/hooks/cli.py"),
                            ("PYTHONPATH", "mcp_servers/build_runtime_server.py"),
                            ("METFORGE_ORCH_LIVENESS_TTL_SECONDS", "tools/validate_workspace_root.py"),
                            ("METFORGE_START_CLAIM_ROOT", "tools/run_workflow.py")):
        ...
        with suite_env_guard.isolated_record():
            with self.assertRaisesRegex(RuntimeError, "without removing it"):
                suite_env_guard.strip_operator_env(
                    _KeepsWhatItPops({"METFORGE_A_KNOB": "1"}))
    ...
    def test_the_stripped_or_declared_decision_reports_an_undecided_name(self) -> None:
        ...
        self.assertEqual(
            suite_env_guard.undecided_environment_names(
                {"METFORGE_A_KNOB", "CODEX_HOME"} | set(MUST_BE_INHERITED)), set())
        self.assertEqual(
            suite_env_guard.undecided_environment_names({"GEMINI_CONFIG_DIR"}),
            {"GEMINI_CONFIG_DIR"})
        self.assertEqual(
            suite_env_guard.undecided_environment_names({"METFORGE2_NEW_KNOB"}),
            {"METFORGE2_NEW_KNOB"},
    ...
    def test_driving_the_strip_leaves_no_global_state_behind(self) -> None:
        ...
        with suite_env_guard.isolated_record():
            ...
            suite_env_guard.STRIPPED_OPERATOR_ENV["METFORGE_SENTINEL"] = "outer"
            with suite_env_guard.isolated_record():
                environ = {"METFORGE_A_KNOB": "1", "PATH": "/b"}
                suite_env_guard.strip_operator_env(environ)
                self.assertTrue(suite_env_guard.CONFIGURED)
                self.assertEqual(suite_env_guard.STRIPPED_OPERATOR_ENV,
                                 {"METFORGE_A_KNOB": "1"})
            self.assertEqual(suite_env_guard.STRIPPED_OPERATOR_ENV,
                             {"METFORGE_SENTINEL": "outer"},
            ...
            del suite_env_guard.STRIPPED_OPERATOR_ENV["METFORGE_SENTINEL"]
    ...
    def test_the_strip_is_reported_and_can_be_declined(self) -> None:
        ...
        `METFORGE_ORCHESTRATION_ENFORCE_LIVE_PREFLIGHT=1` was 84 failures and 482s of real
        ...
        """
        poison = {"METFORGE_ORCHESTRATION_ID": "orch_header_witness"}
        ...
        for extra, expected in (([], 2), (["-q"], 1)):
            with self.subTest(verbosity=extra or ["default"]):
                ...
                self.assertIn("METFORGE_ORCHESTRATION_ID", reported.stdout)
    ...
    def test_a_poisoned_environment_is_neutralized_end_to_end(self) -> None:
        ...
        """
        poison = {"METFORGE_ORCHESTRATION_ID": "orch_witness"}
        node = f"tools/tests/{_SUBJECT_MODULE.rsplit('.', 1)[1]}.py" \
               f"::{_SUBJECT_CLASS}::{_SUBJECT_TEST}"
